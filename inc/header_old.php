...
<html lang="en">
<head>
	<title><?php echo $page_name." | ".$company["name"]; ?></title>
	<meta charset="UTF-8" />
	...
	<script>
		...
		function hide_menu()
		{
			...
			$("li[class*='open'] ul").slideUp();$("li[class*='open']").attr("class","submenu");
...
<body onkeyup="hid_div(event)">

<!--Header-part-->
<div id="header">
	<br>
	<b style="font-size: 28px;color:#fff"><?php echo $company["name"]; ?></b>
</div>
<!--close-Header-part--> 
...
<div id="user-nav" class="navbar navbar-inverse">
	<ul class="nav">
		<li class="dropdown" id="menu-messages"><a href="#" data-toggle="dropdown" data-target="#menu-messages" class="dropdown-toggle"><i class="icon icon-bell"></i> <span class="text">Notification</span> <span class="label label-important">5</span> <b class="caret"></b></a>
			<ul class="dropdown-menu">
				<li onclick="notify_alert()" id="li_exp"><a class="sAdd" title="" href="#">
					<i class="icon-warning-sign"></i> Expiry Notification <span class="badge badge-important" id="not_exp"></span>
				</a></li>
				<li class="divider"></li>
				<li onclick="notify_low_alr()" id="li_stk">
					<a class="sInbox" title="" href="#">
						<i class="icon-exclamation-sign"></i> Low Stock <span class="badge badge-important" id="not_stk"></span>
					</a>
				</li>
			</ul>
		</li>
		<li class="dropdown" id="profile-messages" ><a title="" href="#" data-toggle="dropdown" data-target="#profile-messages" class="dropdown-toggle"><i class="icon icon-user"></i>  <span class="text">Welcome <?php echo $p_info["name"]; ?></span><span id="user" style="display:none;"><?php echo $_SESSION["emp_id"]; ?></span><b class="caret"></b></a>
			<ul class="dropdown-menu">
...
?>
<div id="sidebar"> 
	<a href="" class="visible-phone"><i class="icon-caret-down"></i> Menu List <i class="icon-caret-down"></i></a>
	<ul>
		<li class="<?php echo $active_class; ?>"><a href="index.php"><i class="icon icon-home"></i> <span>Dashboard</span></a> </li>
    ...
    ?>
		 <li class="submenu <?php echo $active_class; ?> "> <a href="#" data-toggle="tooltip" data-placement="right" title="<?php echo $menu_header["name"]; ?>" ><i class="icon icon-th-list"></i> <span><?php echo $menu_header["name"]; ?></span> </a>
			<ul>
			<?php
				...
				while($menu_sub_header=mysqli_fetch_array($menu_sub_header_qry))
				{
					...
					?>
						<li><a href="?param=<?php echo $enc_para; ?>"><?php echo $menu_sub_header["par_name"]; ?></a></li>
					<?php
						}
					}
				}
    ...
    <?php
		...
		if($p_info["branch_id"])
		{
			$our_client=mysqli_fetch_array(mysqli_query($link, " SELECT * FROM `company_name` WHERE `branch_id`='$p_info[branch_id]' limit 0,1 "));
		}
		else
		{
