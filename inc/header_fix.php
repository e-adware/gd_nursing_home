...
<div id="sidebar"> 
    ...
    <?php
		...
		{
			$head_disp="";
			if($main_header["header"]==$menu_header["id"]){ $active_class="active open";$head_disp="style='display:block;'"; }else{ $active_class="";$head_disp="style=''"; }
    ?>
		 <li class="submenu <?php echo $active_class; ?> "> <a href="#"><i class="icon icon-th-list"></i> <span><?php echo $menu_header["name"]; ?></span> </a>
			<ul <?php echo $head_disp;?>>
			<?php
				...
				while($menu_sub_header=mysqli_fetch_array($menu_sub_header_qry))
				{
					...
					if($display=="Yes")
					{
						...
						if($m_num>0)
						{
							...
							if($para==$menu_sub_header["par_id"])
							{
					...
					?>
						<li class="<?php echo $sub_menu_active;?>"><a href="?param=<?php echo $enc_para; ?>"><?php echo $menu_sub_header["par_name"]; ?></a></li>
					<?php
						}
					}
				}
