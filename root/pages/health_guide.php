...
<div id="content-header">
    <div class="header_div"> <span class="header"> Health Agent Master</span></div>
</div>
<!--End-header-->
	...
	<div class="row">
		<div class="span7">
			<table class="table   table-bordered table-condensed">
				...
				<tr>
					<td >Name</td>
					<td ><input type="text" name="intext1" id="txtname" class="intext imp" size="35" value="" autocomplete="off" onKeyup="caps_it(this.value,this.id,event)" /></td>
				</tr>
				<tr>
					<td >Address</td>
				...
				<tr>
					<td >Executive</td>
					<td >
						<select id="sguide_id" onKeyup="sguide_id(this.value,event)">
							<option value="0">Select</option>
						<?php
							$guide_qry=mysqli_query($link, "select * from super_health_guide order by name");
							while($guide=mysqli_fetch_array($guide_qry))
							{
								echo "<option value='$guide[hguide_id]'>$guide[name]</option>";
							}
						?>
						</select>
					</td>
...
<script>
	...
	function popitup(url)
	{
		var txtfrom=0;
		var txtto=0;
		url=url+"?date1="+txtfrom+"&date2="+txtto+"&super=0";
	...
	function val_load_new(doid1)
	{
		...
		function(data,status)
		{
			...
			document.getElementById("sguide_id").value=val[5];
		})
	}
