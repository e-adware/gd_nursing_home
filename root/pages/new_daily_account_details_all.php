...
if($_POST["type"]=="all_account")
{
	...
	if($encounter==0 || $encounter==3 || $encounter==4 || $encounter==5 || $encounter==8)
	{
...
?>	<p style="margin-top: 2%;">
		...
		<!--<span class="text-right" id="excel_btn_hide"><a class="btn btn-info" href="pages/detail_account_export_to_excel.php?date1=<?php echo $date1;?>&date2=<?php echo $date2;?>&encounter=<?php echo $encounter;?>&user_entry=<?php echo $user_entry;?>&pay_mode=<?php echo $pay_mode;?>&account_break=<?php echo $account_break;?>"><i class="icon-file icon-large"></i> Export to Excel</a></span>-->
		
		<!--<button type="button" class="btn btn-info text-right" onclick="print_page('detail','<?php echo $date1;?>','<?php echo $date2;?>','<?php echo $encounter;?>','<?php echo $user_entry;?>','<?php echo $pay_mode;?>','<?php echo $account_break;?>')" style="margin-right: 1%;"><i class="icon-print icon-large"></i> Print</button>-->
		<?php
			//if($c_user==101 || $c_user==102){
		?>
		<button type="button" class="btn btn-info text-right" onclick="print_page('all_account_all','<?php echo $date11;?>','<?php echo $date22;?>','<?php echo $encounter;?>','<?php echo $user_entry;?>','<?php echo $pay_mode;?>','<?php echo $account_break;?>')" style="margin-right: 1%;"><i class="icon-print icon-large"></i> Print</button>
	...
	<?php
		...
		if($user_entry==0)
		{
			...
			{
				// Lab Refund
				$lab_refund_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.refund_amount),0) as maxref from invest_payment_refund a , uhid_and_opdid b where b.`date` between '$date11' and '$date22' and a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and b.type='2' ")); // $refund_max_slno_str_less $refund_max_slno_str_grtr
				// Lab Free
		...
		else
		{
			if($encounter==0 || $encounter==3)
			{
				$ipd_refund_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(refund),0) as maxref from ipd_advance_payment_details where `date` between '$date11' and '$date22' and user='$user_entry' $ipd_max_slno_str_less $ipd_max_slno_str_grtr "));
				//echo "select ifnull(sum(refund),0) as maxref from ipd_advance_payment_details where `date` between '$date11' and '$date22' and user='$user_entry' $ipd_max_slno_str_less $ipd_max_slno_str_grtr ";
			}
			...
			{
				//~ // Lab Refund
				//~ $lab_refund_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.refund_amount),0) as maxref from invest_payment_refund a , uhid_and_opdid b where a.`date` between '$date11' and '$date22' and a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and b.type='2' and a.`user`='$user_entry' $refund_max_slno_str_less $refund_max_slno_str_grtr "));
				//~ // Lab Free
...
?>
	...
	<table class="table table-bordered table-condensed table-report">
		...
		<?php
			...
			?>
		<tr>
		  <td colspan="4"><span class="text-right"><strong>Total Free Amount :</strong></span></td>
		  <td><span class="text-right"><strong><?php echo $rupees_symbol.number_format($tot_cash,2);?> </strong></span></td>
		  <td colspan="4">&nbsp;</td>
		</tr>
	</table>
...
<?php
	...
	{
		$tot_expense_qry=mysqli_fetch_array(mysqli_query($link, "select sum(amount) as tot_exp from expensedetail where expense_date between '$date1' and '$date2' AND `user`='$encounter_val' "));
	}
	$tot_expense=$tot_expense_qry["tot_exp"];
?>
		<tr>
