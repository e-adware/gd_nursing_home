...
$filename ="detail_account_report.xls";
...
if($user_entry>0)
{
	...
	$check_close_account_today=mysqli_fetch_array(mysqli_query($link, " SELECT * FROM `daily_account_close` WHERE `close_date`='$last_date' $user "));	
}
...
<?php
				...
				if($pay_mode=="0") // All payment mode
				{
					...
					if($ipd_casual==0)
					{
						while($inv_pat_pay_detail=mysqli_fetch_array($inv_pat_pay_detail_qry))
						{
							...
							// Refund Amount
							$lab_refund_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.refund_amount),0) as maxref from invest_payment_refund a , uhid_and_opdid b where a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and a.`patient_id`='$all_pat[patient_id]' and a.`opd_id`='$all_pat[opd_id]' and b.type='2' "));
							
							$lab_refund_amount=$lab_refund_val["maxref"];
							
							// Free Amount
							$lab_free_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.free_amount),0) as free_lab from invest_payment_free a , uhid_and_opdid b where a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and a.`patient_id`='$all_pat[patient_id]' and a.`opd_id`='$all_pat[opd_id]' and b.type='2' "));
					...
					?>
						...
						while($pat_pay_detail=mysqli_fetch_array($pat_pay_detail_qry))
						{
							...
							// Refund Amount
							$opd_refund_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.refund_amount),0) as maxref_opd from invest_payment_refund a , uhid_and_opdid b where a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and a.`patient_id`='$all_pat[patient_id]' and a.`opd_id`='$all_pat[opd_id]' and b.type='1' "));
							
							$opd_refund_amount=$opd_refund_val["maxref_opd"];
							
							// Free Amount
							$opd_free_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.free_amount),0) as free_opd from invest_payment_free a , uhid_and_opdid b where a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and a.`patient_id`='$all_pat[patient_id]' and a.`opd_id`='$all_pat[opd_id]' and b.type='1' "));
	...
	else
	{
		...
		// Lab Refund
		$lab_refund_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.refund_amount),0) as maxref from invest_payment_refund a , uhid_and_opdid b where a.`date` between '$date11' and '$date22' and a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and b.type='2' and b.`user`='$user_entry' "));
		// Lab Free
		$lab_free_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.free_amount),0) as lab_free from invest_payment_free a , uhid_and_opdid b where a.`date` between '$date11' and '$date22' and a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and b.type='2' and b.`user`='$user_entry' "));
		// OPD Refund
		$opd_refund_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.refund_amount),0) as maxref_opd from invest_payment_refund a , uhid_and_opdid b where a.`date` between '$date11' and '$date22' and a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and b.type='1' and b.`user`='$user_entry' "));
		// OPD Free
		$opd_free_val=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(a.free_amount),0) as opd_free from invest_payment_free a , uhid_and_opdid b where a.`date` between '$date11' and '$date22' and a.`patient_id`=b.`patient_id` and a.`opd_id`=b.`opd_id` and b.type='1' and b.`user`='$user_entry' "));
	...
	<tr>
		...
		<td><?php echo $rupees_symbol.number_format($tot_amt_card,2); ?></td>
		<td></td>
		<td></td>
	</tr>
	<?php } } ?>
	...
	?>
		<tr>
			...
			<td><?php echo $rupees_symbol.number_format($tot_amt_cash,2); ?></td>
			<td></td>
			<td></td>
		</tr>
	<?php
	...
	?>
		<tr>
			...
			<td><?php echo $rupees_symbol.number_format($tot_amt,2); ?></td>
			<td></td>
			<td></td>
		</tr>
	...
	<?php if($pay_mode=="0"){ ?>
		<tr>
			...
			<td><?php echo $rupees_symbol.number_format($tot_expense,2); ?></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
			<th colspan="5"><span class="text-right">Total Refund</span></th>
			<td><?php echo $rupees_symbol.number_format($ipd_refund+$total_refund_amount,2); ?></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
			<th colspan="5"><span class="text-right">Total Free</span></th>
			<td><?php echo $rupees_symbol.number_format($total_free_amount,2); ?></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
			<th colspan="5"><span class="text-right">Net Amount</span></th>
			<td><?php echo $rupees_symbol.number_format(($tot_amt-$tot_expense-$ipd_refund-$total_refund_amount-$total_free_amount),2); ?></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
			<th colspan="5"><span class="text-right">Net Cash</span></th>
			<td><?php echo $rupees_symbol.number_format(($tot_amt-$tot_expense-$ipd_refund-$total_refund_amount-$total_free_amount-$tot_amt_card),2); ?></td>
			<td></td>
			<td></td>
		</tr>
<?php
