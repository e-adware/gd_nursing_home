...
if($_POST["type"]=="doctor_account")
{
	...
	$pat_typ_text=mysqli_fetch_array(mysqli_query($link, " SELECT `p_type`,`type` FROM `patient_type_master` WHERE `p_type_id`='$dept_id' "));
	$encounter=$pat_typ_text["type"];
?>
	<p id="print_div" style="margin-top: 2%;"><b>Doctor Account from:</b> <?php echo convert_date($date1)." <b>to</b> ".convert_date($date2); ?>
...
<?php
		...
		if($dept_id==0 || $dept_id==1)
		{
			...
			if($pat_reg_num!=0)
			{
				echo "<tr><th colspan='8'>OPD Account</th></tr>";
		?>	
			<?php
				$n=1;
	...
	<?php
		...
		if($dept_id==0 || $dept_id==3)
		{
			...
			if($pat_reg_num!=0)
			{
				echo "<tr><th colspan='8'>IPD Account</th></tr>";
		?>	
			<?php
				$n=1;
				$ipd_tot_bill_amout=0;
					...
					else
					{
						...
						}else
						{
							...
							}else
							{
								...
								$ipd_service_bill=mysqli_fetch_array(mysqli_query($link, "SELECT `amount` FROM `ipd_pat_service_details` WHERE `patient_id`='$pat_reg[patient_id]' AND `ipd_id`='$pat_reg[ipd_id]' AND `service_id`='$pat_reg[service_id]' AND `slno`='$pat_reg[rel_slno]'"));
								$bill_amount=$ipd_service_bill["amount"];
							}
						}
						$show_service="Yes";
	...
	<?php
		...
		if($dept_id==0 || $dept_id==4)
		{
			...
			if($pat_reg_num!=0)
			{
				echo "<tr><th colspan='8'>Casualty Account</th></tr>";
		?>	
			<?php
				$i=$n=1;
				$casu_tot_bill_amout=0;
	...
	<?php
		...
		if($dept_id==0 || $dept_id==5)
		{
			...
			if($pat_reg_num!=0)
			{
				echo "<tr><th colspan='8'>Day Care Account</th></tr>";
		?>	
			<?php
				$i=1;
				$n=1;
