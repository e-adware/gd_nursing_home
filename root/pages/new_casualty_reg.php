...
$at_doc_name=mysqli_fetch_array(mysqli_query($link, " SELECT `consultantdoctorid`,`Name` FROM `consultant_doctor_master` WHERE `consultantdoctorid`='$pat_emergency_det[consultantdoctorid]' "));

$at_doc=$at_doc_name["Name"];
$atdoc_id=$at_doc_name["consultantdoctorid"];
if(!$atdoc_id){ $atdoc_id=0; }
		...
		<div class="doctor_info_div" style="<?php echo $edit_payment_style; ?>">
			...
			<table id="doctor_info_tbl" class="table table-condensed" style="background-color:#FFF">
				...
				<tr>
					...
					<th class="span3">Consultant Doctor <b style="color:#ff0000;">*</b></th>
					<td>
						<input type="text" name="at_doc" id="at_doc" class="span2 at_doc pat_info" size="25" onfocus="atload_doc_focus()" onKeyUp="atload_doc_up(this.value,event)" onBlur="javascript:$('#atref_doc').fadeOut(500)" value="<?php echo $at_doc; ?>" />
						<input type="hidden" name="atdoc_id" id="atdoc_id" value="<?php echo $atdoc_id; ?>" />
						...
						<div id="atref_doc" align="center">
							<table style="background-color:#FFF" class="table table-bordered table-condensed" id="center_table" width="300px">
								...
								<th>Doctor Name</th>
								<?php
									$d=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT `consultantdoctorid`,`Name` FROM `consultant_doctor_master` ORDER BY `Name`"); // `dept_id`=46
									$i=1;
									while($d1=mysqli_fetch_array($d))
									{
								?>
					...
					<th class="span3">Emergency Type <b style="color:#ff0000;">*</b></th>
					<td colspan="1">
				...
				<tr class="police_detail" <?php echo $mlc_style; ?>>
					<th>Police Station</th>
					<td>
						<input type="text" class="span3" id="police_detail_station" value="<?php echo $pat_emergency_det["police_station"]; ?>" onKeyup="police_detail_station_up(event,this.value,this.id)" list="police_detail_station_datalist">
...
<script>
	...
	{
		$.post("pages/new_casualty_registration_data.php",
		{
			type:"load_center",
	...
	function police_detail_station_up(e,val,id)
	{
		...
		if(unicode==13)
		{
			$("#police_detail_officer").focus();
		}
	}
	function police_detail_officer_up(e,val,id)
	{
		var nval=val.toUpperCase();
		$("#"+id).val(nval);
		...
		if(unicode==13)
		{
			$("#police_detail_case_no").focus();
		}
	}
	function police_detail_case_no_up(e,val,id)
	{
		var unicode=e.keyCode? e.keyCode : e.charCode;
		if(unicode==13)
		{
			$("#police_detail_date").focus();
		}
	}
	function police_detail_date_up(e)
	{
		var unicode=e.keyCode? e.keyCode : e.charCode;
		if(unicode==13)
		{
			$("#police_detail_time").focus();
		}
	}
	function police_detail_time_up(e)
	{
		var unicode=e.keyCode? e.keyCode : e.charCode;
		if(unicode==13)
		{
			$("#police_detail_desc").focus();
		}
	}
	function police_detail_desc_up(e,val,id)
	{
		var unicode=e.keyCode? e.keyCode : e.charCode;
		if(unicode==13)
		{
	...
	function pat_save()
	{
		...
		{
			scrollPage(scroll_val)
			$("#at_doc").val("").focus().css({"border-color":"red"});;
			return false;
		}
		if($("#pat_visit_type").val()=="0")
		{
		...
		function(data,status)
		{
			...
			setTimeout(function(){
				...
				if(res[1]!="0")
				{
					window.location="processing.php?param=85&uhid="+res[0]+"&ipd="+res[1];
				}
			},2000);
		})
	}
