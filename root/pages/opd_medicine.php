...
<div id="content-header">
    <div class="header_div">
		<span class="header">Patients List</span>
	</div>
</div>
<!--End-header-->
		 ...
		 <tr>
			...
			<td class="">
				...
				<?php
					...
					$pid = mysqli_query($link," SELECT `uhid` FROM `patient_info` WHERE `patient_id` IN (SELECT `patient_id` FROM `uhid_and_opdid` WHERE `type`='1') order by `slno` DESC");
...
<script>
	...
	function redirect_page(uhid,opd)
	{
		window.location="processing.php?param=103&uhid="+uhid+"&opd="+opd;
	}
</script>
