...
?>
	<table id="padd_tbl" class="table table-condensed">
		...
		<tr>
			<th>Consultant Doctor <b style="color:#ff0000;">*</b></th>
			<td>
		...
		<tr>
			<th>Referred By</th>
			<td colspan="">
				<input type="text" name="r_doc" id="r_doc" class="span2" size="25" onFocus="load_refdoc1()" onKeyUp="load_refdoc(this.value,event,'ipd')" onBlur="javascript:$('#ref_doc').fadeOut(500)" value="<?php echo $ref_doc_name["ref_name"]."-".$ref_doc_name["refbydoctorid"]; ?>">
				<input type="text" name="doc_id" id="doc_id" style="display:none;" value="<?php echo $ref_doc_name["refbydoctorid"]; ?>">
...
?>
	...
	<table id="padd_tbl" class="table table-condensed">
		...
		<th>Referred By <b style="color:#ff0000;">*</b></th>
			...
			<th>Assign Bed <b style="color:#ff0000;">*</b> <input type="text" style="display:none;" id="bed_id" /></th>
			<td colspan="3"><button type="button" id="bedbtn" class="btn btn-search" onclick="load_bed_details()">Click to view</button></td>
			<th style="display:none;">Card Type</th>
			<td style="display:none;">
				<select id="card_id" onkeyup="card_id_lab(this.value,event)">
			<?php
...
<script src="../jss/ipd_reg.js"></script>
...
<script>
	...
	function save(typ)
	{
		...
		else if($("#g_name").val()=="") // edit
		{
			$("#g_name").css('border','2px solid #ff0000');
			$("#g_name").focus();
		}
		else if($("#g_ph").val()=="") // edit
		{
			$("#g_ph").css('border','2px solid #ff0000');
			$("#g_ph").focus();
		}
		else if($("#city").val()=="") // edit
		{
		...
		else
		{
			...
			function(data,status)
			{
				...
				if(vl[2]==0)
				{
					...
					setTimeout(function()
					{
						window.location="processing.php?param=52&uhid="+vl[0]+"&ipd="+vl[1];
					 }, 1000);
				 }else
				 {
					 save(typ);
