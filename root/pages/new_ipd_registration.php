...
$pat_doc=mysqli_fetch_array(mysqli_query($link, " SELECT * FROM `ipd_pat_doc_details` WHERE `patient_id`='$uhid' and `ipd_id`='$opd_id' "));

$at_doc_name=mysqli_fetch_array(mysqli_query($link, " SELECT `consultantdoctorid`,`Name` FROM `consultant_doctor_master` WHERE `consultantdoctorid`='$pat_doc[attend_doc]' "));

$at_doc=$at_doc_name["Name"];
$atdoc_id=$at_doc_name["consultantdoctorid"];
if(!$atdoc_id){ $atdoc_id=0; }

$ad_doc_name=mysqli_fetch_array(mysqli_query($link, " SELECT `consultantdoctorid`,`Name` FROM `consultant_doctor_master` WHERE `consultantdoctorid`='$pat_doc[admit_doc]' "));

$ad_doc=$ad_doc_name["Name"];
$addoc_id=$ad_doc_name["consultantdoctorid"];
if(!$addoc_id){ $addoc_id=0; }

$pat_bed=mysqli_fetch_array(mysqli_query($link, " SELECT `bed_id` FROM `ipd_pat_bed_details` WHERE `patient_id`='$uhid' and `ipd_id`='$opd_id' "));
$bed_id=$pat_bed["bed_id"];
if(!$bed_id){ $bed_id=0; }
		...
		<div class="doctor_info_div" style="<?php echo $edit_payment_style; ?>">
			...
			<table id="doctor_info_tbl" class="table table-condensed" style="background-color:#FFF">
				...
				<tr>
					...
					<td>
						<input type="text" name="at_doc" id="at_doc" class="span2 at_doc pat_info" size="25" onFocus="atload_refdoc1()" onKeyUp="atload_refdoc(this.value,event)" onBlur="javascript:$('#atref_doc').fadeOut(500)" value="<?php echo $at_doc; ?>" />
					...
					<th class="span3">Admitted By <b style="color:#ff0000;">*</b></th>
					<td>
						<input type="text" name="ad_doc" id="ad_doc" class="ad_doc pat_info" onFocus="con_load_focus()" onKeyUp="con_doc_load(this.value,event)" onBlur="javascript:$('#adref_doc').fadeOut(500)" value="<?php echo $ad_doc; ?>">
						<input type="hidden" id="addoc_id" value="<?php echo $addoc_id; ?>">
				...
				<tr>
					<th>Department <b style="color:#ff0000;">*</b></th>
					<td colspan="5">
						<select id="dept_id" class="span3" onkeyup="dept_id_up(this,event)">
							<option value="0">Select</option>
						<?php
							$dept_qry=mysqli_query($link, " SELECT * FROM `doctor_specialist_list` order by `name` ");
							while($dept=mysqli_fetch_array($dept_qry))
							{
								if($pat_doc["dept_id"]==$dept["speciality_id"]){ $dept_sel="selected"; }else{ $dept_sel=""; }
				...
				<tr style="<?php echo $bed_tr_str; ?>">
					<th>Assign Bed <b style="color:#ff0000;">*</b></th>
					<td colspan="5">
						<button type="button" id="bedbtn" class="btn btn-search" onclick="load_bed_details()">Click to view</button>
						<input type="text" style="display:none;" id="bed_id" value="<?php echo $bed_id; ?>" />
					</td>
				</tr>
				<tr id="save_tr">
...
<script>
	...
	function con_doc_load(val,e)
	{
		...
		else
		{
			...
			addoc_load(con_doc_id,doc_naam);
		}
	}
	...
	function pat_save()
	{
		...
		if($("#bed_id").val()=="0" || $("#bed_id").val()=="")
		{
			scrollPage(scroll_val)
			$("#bedbtn").css({"border-color":"red"}).focus();
			return false;
		}
		...
		function(data,status)
		{
			...
			setTimeout(function(){
				...
				if(res[1]!="0")
				{
					window.location="processing.php?param=52&uhid="+res[0]+"&ipd="+res[1];
				}
			},2000);
		})
	}
