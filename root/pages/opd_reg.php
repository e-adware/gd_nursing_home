...
?>
	<table id="padd_tbl" class="table table-condensed">
		...
		<tr>
			...
			<th>Police Station <b style="color:#ff0000;">*</b></th>
			<td>
...
?>
	...
	<table id="padd_tbl" class="table table-condensed">
		...
		<tr>
			...
			<th>Bill No</th>
			<td>
				<input type="text" id="search_pin" onkeyup="load_emp(this.value,event,'pin')" placeholder="Type Bill No" >
			</td>
			<th>Name</th>
			<td>
...
<script src="../jss/opd_reg.js"></script>
...
<script>
	...
	function save(typ)
	{
		...
		//~ else if($(".dob").val()=="" || $(".dob").val().length!=10)
		//~ {
			//~ $(".dob").css('border','2px solid #ff0000');
			//~ $(".dob").focus();
		//~ }
		...
		{
			$("#dist").css('border','2px solid #ff0000');
			$("#dist").focus();
		}
		else if($("#police").val()=="")
		{
			$("#police").css('border','2px solid #ff0000');
			$("#police").focus();
		}
		else
		{
			...
			function(data,status)
			{
				...
				if(str[0]=='0')
				{
					...
					setTimeout(function()
					{
						window.location="processing.php?param=3&uhid="+str[1]+"&consult=1&opd="+opd_id+param_str;
