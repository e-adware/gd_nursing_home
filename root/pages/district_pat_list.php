		...
		<tr>
			<td>
				<center>
					...
					<?php
						$con_doc_qry=mysqli_query($link, "SELECT * FROM `district` WHERE `state_id`='4' ORDER BY `name` ");
						while($con_doc=mysqli_fetch_array($con_doc_qry))
						{
							echo "<option value='$con_doc[district_id]'>$con_doc[name]</option>";
						}
					?>
					</select>
					<br>
...
<script>
	...
	{
		$("#loader").show();
		$.post("pages/district_pat_list_qry.php",
		{
	...
	function print_page(dt1,dt2,dist)
	{
		url="pages/district_pat_list_print.php?fdate="+dt1+"&tdate="+dt2+"&district="+dist;
