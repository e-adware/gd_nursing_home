...
if($pat_info["dob"]!=""){ $age=age_calculator_date($pat_info["dob"],$reg_date)."(".$pat_info["dob"].")"; }else{ $age=$pat_info["age"]." ".$pat_info["age_type"]; }
...
<div id="content-header">
    <div class="header_div"> <span class="header">Baby Dashboard</span></div>
</div>
<!--End-header-->
<div class="container-fluid">
	<span style="float:right;">
		<button class="btn btn-back" id="back_btn" onclick="window.location='processing.php?param=118'"><i class="icon-backward"></i> Back</button>
	</span>
	<table class="table table-condensed table-bordered" style="background: snow">
		...
		<tr>
			...
			<td><?php echo $delivery_det['patient_id'];?></td>
			<td><?php echo $delivery_det['ipd_id'];?></td>
		</tr>
	</table>
	...
	<div class="" style="">
		...
		<?php
			$accordian_qry=mysqli_query($link, "SELECT `accordian_id`, `accordian_name` FROM `nursing_dashboard_menu` WHERE `accordian_id` IN(3) AND `status`=0 ORDER BY `sequence` ASC");
			...
			<div class="accordion-group widget-box" style="display:;">
				<div class="accordion-heading">
					<div class="widget-title">
						<a data-parent="#collapse-group" id="collapse_data<?php echo $accordian_info["accordian_id"]; ?>" href="#collapse<?php echo $accordian_info["accordian_id"]; ?>" data-toggle="collapse" onclick="show_icon(<?php echo $accordian_info["accordian_id"]; ?>)" class="<?php echo $cls_name; ?>">
							<span class="icon" style="width:90%;"><b style="padding:10px;font-size:16px;"><?php echo $accordian_info["accordian_name"]; ?></b><i class="icon-arrow-down" id="ard<?php echo $accordian_info["accordian_id"]; ?>"></i><i class="icon-arrow-up" id="aru<?php echo $accordian_info["accordian_id"]; ?>" style="display:none;"></i></span>
							<span class="text-right" style="padding:10px;font-size:18px;">
								<span class="iconp" id="plus_sign<?php echo $accordian_info["accordian_id"]; ?>" style="float:right;"><i class="icon-plus"></i></span>
								<span class="iconm" id="minus_sign<?php echo $accordian_info["accordian_id"]; ?>" style="float:right;display:none;"><i class="icon-minus"></i></span>
							</span>
						</a>
						<span class="icon" id="collapse_none<?php echo $accordian_info["accordian_id"]; ?>" style="width:90%;display:none;cursor:not-allowed;"><b style="padding:10px;font-size:16px;"><?php echo $accordian_info["accordian_name"]; ?></b>
					</div>
				</div>
				<div class="accordion-body collapse" id="collapse<?php echo $accordian_info["accordian_id"]; ?>" style="height:0px;max-height:700px;overflow-y:scroll;">
					<div class="widget-content hidden_div" id="accordian_data_load<?php echo $accordian_info["accordian_id"]; ?>" style="display:none;">
...
<button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#bed_modal" id="bed_modal_btn" style="display:none"></button>
<div id="bed_modal" class="modal fade" role="dialog" style="border-radius:0;display:none">
		...
		<div id="myAlert1" class="modal fade">
			...
			<div class="modal-footer">
				<center>
					...
					<button class="btn btn-close" id="modal_test_close_btn" data-dismiss="modal"><i class="icon-off"></i> Close</button>
				</center>
			</div>
		</div>
	<!--modal end-->
	...
	<input type="text" id="modtxt" value="0" style="display:none"/>
	<div class="modal fade" id="myModal_inv_entry" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="border-radius:0;">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<button type="button" class="bootbox-close-button close" onclick="$('#med_list_post').css('height','100px')" data-dismiss="modal" aria-hidden="true"><b>x</b></button>
...
function edit_certficate(slno)
{
	...
	$(".clk_tr").css({"background":"","color":"#444"});
	$("#loader").show();
	...
	function(data,status)
	{
		//alert(data);
		$("#loader").hide();
		$("#clk"+slno).css({"background":"#FBFBD4","color":"#000"});
		$("#tr"+slno).show();
		$("#sl"+slno).html(data);
...
function pat_ipd_delivery_save()
{
	...
	for(var i=1;i<=no;i++)
	{
		...
		all+=$("#dob"+i).val()+"@"+$("#sex"+i).val()+"@"+b_time+"@"+$("#wt"+i).val()+"@"+$("#blood"+i).val()+"@"+$("#dmode"+i).val()+"@"+$("#conducted"+i).val()+"@"+$("#tag"+i).val()+"@"+fat_name+"@##";
	}
	$("#loader").show();
...
function load_items(id,name,rate)
{
	...
	function(data,status)
	{
		...
		$("#ss_tests").animate({ scrollTop: 2900 });
...
function save_test()
{
	...
	function(data,status)
	{
		bootbox.dialog({ message: "<h4>Saved</h4>"});
		setTimeout(function()
		{
			bootbox.hideAll();
...
function load_test_new_old(id,name,rate)
{
	...
	if(test_chk==0)
	{
		...
		test_add+="<tr><th style='background-color:#cccccc'>#</th><th style='background-color:#cccccc'>Tests</th><th style='background-color:#cccccc'>Rate</th><th style='background-color:#cccccc'>Remove</th></tr>";
		test_add+="<tr><td>1</td><td width='80%'>"+name+"<input type='hidden' value='"+id+"' class='test_id'/></td><td><span class='test_f'>"+rate+"</span></td><td onclick='delete_rows(this,2)'><span class='text-danger' style='cursor:pointer'><i class='icon-remove'></i></span></td></tr>";
		test_add+="</table>";
...
////-------------------------------------------------------------//
	...
	function nursing_bed_transfer(val)
	{
		...
		function(data,status)
		{
			...
			$("#bed_modal").animate({'top':'1%','left':'23%',"width":"95%",'margin':'auto'},"slow");
...
///-------------------------------------------------------------//
	...
	function add_row(r)
	{
		...
		{
			$("#loader").hide();
			$("#hh").closest("tr").before('<tr class="cc" id="tr'+rr+'"><th>Chief Complaints</th><td><input list="browsrs'+rr+'" type="text" id="chief'+rr+'" class="" onkeyup="sel_chief(this.id,'+rr+',event)" /><span id="comp_lst'+rr+'"></span></td><td><b>for</b> <select id="cc'+rr+'" class="span2" onkeyup="sel_chief(this.id,'+rr+',event)"><option value="0">--Select--</option>'+d+'</select> <select id="tim'+rr+'" class="span2" onkeyup="sel_chief(this.id,'+rr+',event)"><option value="0">--Select--</option>'+s+'</select></td><td><span style="float:right"><button type="button" class="btn-mini btn-danger" onclick="$(this).parent().parent().parent().remove()"><span class="icon-remove"></span></button></span></td></tr>');
	...
	function room_status()
	{
		...
		function(data,status)
		{
			...
			$("html,body").animate({scrollTop: '160px'},800);
	...
	*/
	...
	function ad()
	{
		...
		function(data,status)
		{
			...
			$('#diag_table').append('<tr id="tr'+rr+'"><td><input list="browsr'+rr+'" type="text" class="span4" onkeyup="diagtab(1,event)" id="diagnosis1" placeholder="Diagnosis" /><span id="diagn'+rr+'">'+vl[0]+'</span></td><td><select id="order1" onkeyup="diagtab(1,event)" class="span2"><option value="0">--Select--</option><option value="Primary">Primary</option><option value="Secondary">Secondary</option></select></td><td><select id="cert1" onkeyup="diagtab(1,event)" class="span2"><option value="0">--Select--</option><option value="Confirmed">Confirmed</option><option value="Presumed">Presumed</option></select></td><td><select id="doc"><option value="0">Select</option>'+vl[1]+'</select></td><td><span style="float:right"><button type="button" class="btn-mini btn-danger" onclick="$(this).parent().parent().parent().remove()"><span class="icon-remove"></span></button></span></td></tr>');
		})
	}
	...
	{
		var user=$("#user").text().trim();
		url="pages/ipd_batch_wise_test_bill.php?uhid="+btoa(uhid)+"&ipdid="+btoa(ipd)+"&batch="+btoa(batch)+"&user="+btoa(user);
		window.open(url,'Window','scrollbars=1,menubar=1,toolbar=0,height=670,width=1000');
	}
	...
	function sample_accept(pid,opd,ipd,batch_no)
	{
		...
		function(data,status)
		{
			...
			setTimeout(function()
			{
				...
				bootbox.hideAll();
				var user=$("#user").text();
				var url="pages/barcode_generate.php?pid="+pid+"&opd_id="+opd+"&ipd_id="+ipd+"&batch_no="+batch_no+"&user="+user+"&vac="+vac;
	...
	{
		var user=$("#user").text();
		var url="pages/barcode_generate.php?pid="+pid+"&opd_id="+opd+"&ipd_id="+ipd+"&batch_no="+batch_no+"&user="+user+"&vac="+vc+"&sing="+1;	
		window.open(url,'','fullscreen=yes,scrollbars=yes');
	}
	...
	{
		var user=$("#user").text().trim();
		url="pages/phlebo_gen_req_ipd.php?uhid="+uhid+"&ipd="+ipd+"&batch="+batch+"&dep="+dep+"&user="+user;
		window.open(url,'Window','scrollbars=1,menubar=1,toolbar=0,height=670,width=1000');
	}
...
<style>
	...
	#myAlert, #myAlert1, #myModal_med, #medplan, #myModal_post, #repmodal
	{
	    width: 80%;
		margin-left: -40%;
	}
