...
if($pat_info && $pat_reg)
{
	...
	if($cancel_request)
	{
		...
		$tr_title="Cancel request by ".$emp_info_del["name"];
	}
	else
	{
		$tr_title="";
	...
	$ref_request=mysqli_fetch_array(mysqli_query($link,"SELECT * FROM `refund_request` WHERE `patient_id`='$uhid' AND `opd_id`='$opd_id' AND `status`='0' "));
	if($ref_request)
	{
	...
	echo "<input type='hidden' id='pateint_type' value='$pat_reg[type]'>";
...
<div id="content-header">
    <div class="header_div"> <span class="header">Refund Request</span></div>
</div>
<!--End-header-->
<link rel="stylesheet" href="../css/loader.css" />
<div class="container-fluid">
	<span style="float:right;">
		<button class="btn btn-back" id="add" onclick="window.location='processing.php?v=0&param=310'"><i class="icon-backward"></i> Back</button>
	</span>
	<div class="">
		<div class="">
...
<script>
	...
	function load_ref_requested_data()
	{
		$.post("pages/refund_request_ajax.php",
		{
			type:"load_ref_requested_data",
			uhid:$("#patient_id").val(),
			opd_id:$("#opd_id").val(),
	...
	{
		if($(".refund_type:checked").val()==1)
		{
			if($(dis).is(":checked"))
			{
				$(".addon_"+$(dis).val()).prop("checked", true);
			}
			else
			{
				$(".addon_"+$(dis).val()).prop("checked", false);
			}
		}
	...
	function save_refund()
	{
		...
		for(var i=0;i<ref_ser.length;i++)
		{
			...
			var group_id=$("#group_id"+ser_id).val();
			...
			sel_services=sel_services+"@@"+group_id+"$$"+ser_id+"$$"+ref_amount+"$$"+fix_amount+"$$"+actual_amount;
		}
		if($("#ref_reason").val()=="")
		{
		...
		bootbox.dialog({
			...
			message: "<h5>Are you sure want to send refund request ?</h5>",
			buttons: {
				cancel: {
				...
				confirm: {
					...
					callback: function() {
						...
						$.post("pages/refund_request_ajax.php",
						{
							type:"save_refund_request",
							uhid:$("#patient_id").val(),
							opd_id:$("#opd_id").val(),
							pateint_type:$("#pateint_type").val(),
						...
						function(data,status)
						{
							...
							if(res[1]!=0)
							{
								$("#refund_type_all").val($(".refund_type:checked").val());
								$("#refund_request_id").val(res[1]);
								check_refund_request();
							}
							else
							{
								$("#ref_requested_data").html("");
								$("#save_refund_btn").show();
							}
							setTimeout(function(){
								bootbox.hideAll();
	...
	{
		$("#cancel_refund_btn").hide();
		bootbox.dialog({
			//title: "Patient Re-visit ?",
			message: "<h5>Are you sure want to cancel refund request ?</h5>",
			buttons: {
				cancel: {
					label: '<i class="icon-remove"></i> No',
					className: "btn btn-inverse",
	...
	function reset_refund_request()
	{
		$("#refund_type_all").val("0");
		$("#refund_request_id").val("0");
		$(".refund_type").prop("disabled", false).prop("checked", false);
		$(".refund_select").prop("disabled", false).prop("checked", false);
		refund_type_change("0");
		$("#ref_reason").prop("disabled", false).val("");
		$("#save_refund_btn").show();
		$("#ref_requested_data").html("");
