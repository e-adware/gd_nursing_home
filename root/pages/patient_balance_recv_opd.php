...
if($pat_pay_det)
{
	...
	if($balance>=0)
	{
		...
		if(mysqli_query($link, " INSERT INTO `consult_payment_detail`(`patient_id`, `opd_id`, `bill_no`, `payment_mode`, `typeofpayment`, `amount`, `balance`, `discount`, `discount_reason`, `refund`, `refund_reason`, `cheque_ref_no`, `user`, `time`, `date`) VALUES ('$uhid','$opd_id','$bill_no','$pay_mode','B','$now_pay','$balance','$discount','$discount_reason','$refund_amount','$refund_reason','$cheque_ref_no','$user','$time','$date') "))
		{
			mysqli_query($link, "update consult_patient_payment_details set dis_amt='$dis_amt',advance='$total_advance',balance='$balance' where patient_id='$uhid' and opd_id='$opd_id'");
