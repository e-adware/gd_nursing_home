...
$v=mysqli_fetch_assoc(mysqli_query($link,"SELECT * FROM `inv_main_stock_received_master` WHERE `receipt_no`='$rcv'"));
//$v=mysqli_fetch_assoc(mysqli_query($link,"SELECT * FROM `ph_purchase_receipt_master` WHERE `order_no`='$rcv'"));
?>
<!--header-->
<div id="content-header">
    <div class="header_div"> <span class="header">Purchase Received Bill Edit</span></div>
</div>
<!--End-header-->
...
{
	$("#loader").show();
	$.post("pages/inv_add_purchase_item_ajax.php"	,
	{
		rcv:$("#rcv_no").val().trim(),
		type:2,
	},
...
function save_data_final()
{
	...
	else
	{
		...
		//alert(all);
		var olds=""
		var lens=$(".all_tr").length;
		for(var i=0; i<lens; i++)
		{
			olds+=$(".all_tr:eq("+i+")").find('td:eq(1) input:first').val(); // item_id
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(2) input:first').val(); // bch
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(3) input:first').val(); // exp_dt
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(4) input:first').val(); // qnt
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(5) input:first').val(); // free
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(6) input:first').val(); // gst
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(6) input:last').val(); // gst_amt
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(7) input:first').val(); // pkd_qnt
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(8) input:first').val(); // strip mrp
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(8) input:last').val(); // unit_sale
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(9) input:first').val(); // strip cost
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(9) input:last').val(); // unit_cost
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(10) input:first').val(); // disc_per
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(10) input:last').val(); // d_amt
			olds+="@@"+$(".all_tr:eq("+i+")").find('td:eq(11) input:first').val(); // itm_amt
			olds+="@@#%#";
		}
		//alert(olds);
		$.post("pages/inv_add_purchase_item_ajax.php",
