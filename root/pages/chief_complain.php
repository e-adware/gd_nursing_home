<script>
	$(document).ready(function()
	{
		
	});
	function load_complain()
	{
		$.post("pages/chief_complain_ajax.php",
		{
			srch:$("#srch").val(),
			type:"load_complain",
		},
		function(data,status)
		{
			$("#res").html(data);
		})
	}
	function save()
	{
		if($("#name").val()=="")
		{
			$("#name").focus();
		}
		else
		{
			$("#sav").attr('disabled',true);
			$.post("pages/chief_complain_ajax.php",
			{
				id:$("#id").val(),
				dept:$("#dept").val(),
				name:$("#name").val(),
				type:"save_complain",
			},
			function(data,status)
			{
				$("#sav").attr('disabled',false);
				bootbox.dialog({ message: data});
				setTimeout(function()
				{
					bootbox.hideAll();
					clrr();
				}, 1000);
			})
		}
	}
	function edt(id)
	{
		$.post("pages/chief_complain_ajax.php",
		{
			id:id,
			type:"edit_complain",
		},
		function(data,status)
		{
			var vl=data.split("@govin@");
			$("#id").val(vl[0]);
			$("#dept").val(vl[1]);
			$("#name").val(vl[2]);
			$("#sav").val('Update');
			$("#dept").focus();
		})
	}
	function del(id)
	{
		$("#dl").click();
		$("#idl").val(id);
	}
	function delete_complain()
	{
		$.post("pages/chief_complain_ajax.php",
		{
			id:$("#idl").val(),
			type:"delete_complain",
		},
		function(data,status)
		{
			bootbox.dialog({ message: data});
			setTimeout(function()
			{
				bootbox.hideAll();
				clrr();
			}, 1000);
		})
	}
	function clrr()
	{
		$("#id").val('');
		$("#dept").val('0').change();
		$("#name").val('');
		$("#srch").val('');
		$("#sav").val('Save');
		$("#dept").focus();
		load_complain();
	}
</script>
<!--header-->
<div id="content-header">
    <div class="header_div"> <span class="header"> <?php echo $menu_info["par_name"]; ?></span></div>
</div>
<!--End-header-->
<div class="container-fluid">
	<div class="span5" style="margin-left: 0;">
		<table class="table table-bordered table-condensed">
			<tr>
				<td>Department</td>
				<td>
					<input type="text" id="id" style="display:none;" />
					<select id="dept" autofocus>
						<option value="0">Select</option>
						<?php
						$q=mysqli_query($link,"SELECT * FROM `doctor_specialist_list` ORDER BY `name`");
						while($r=mysqli_fetch_array($q))
						{
						?>
						<option value="<?php echo $r['speciality_id'];?>"><?php echo $r['name'];?></option>
						<?php
						}
						?>
					</select>
				</td>
			</tr>
			<tr>
				<td>Complain Name</td>
				<td>
					<input type="text" id="name" class="span3" placeholder="Complain Name" />
				</td>
			</tr>
			<tr>
				<td colspan="2" style="text-align:center;">
					<input type="button" id="sav" class="btn btn-info" onclick="save()" value="Save" />
					<input type="button" id="" class="btn btn-danger" onclick="clrr()" value="Reset" />
				</td>
			</tr>
		</table>
	</div>
	<div class="span6">
		<b>Search</b> <input type="text" id="srch" onkeyup="load_complain()" class="span4" placeholder="Search..." />
		<div id="res" style="max-height:400px;overflow-y:scroll;">
		
		</div>
	</div>
		<!--modal-->
			<a href="#myAlert" data-toggle="modal" id="dl" class="btn" style="display:none;">A</a>
            <div id="myAlert" class="modal fade">
              <div class="modal-body">
				  <input type="text" id="idl" style="display:none;" />
                <p>Are You Sure Want To Delete...?</p>
              </div>
              <div class="modal-footer">
				<a data-dismiss="modal" onclick="delete_complain()" class="btn btn-primary" href="#">Confirm</a>
				<a data-dismiss="modal" onclick="clrr()" class="btn btn-info" href="#">Cancel</a>
			  </div>
            </div>
          <!--modal end-->
	<script>
		load_complain();
	</script>
</div>
