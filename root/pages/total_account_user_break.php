	...
	<table class="table table-bordered text-center">
		<tr>
			<td>
				<center>
					<b>From</b>
					<input class="form-control datepicker span2" type="text" name="from" id="from" value="<?php echo date("Y-m-d"); ?>" <?php echo $dept_sel_dis; ?> >
					<b>To</b>
					<input class="form-control datepicker span2" type="text" name="to" id="to" value="<?php echo date("Y-m-d"); ?>" <?php echo $dept_sel_dis; ?> >
...
-->
					...
					<select id="account_break" class="span2">
						<option value="0">Now</option>
					<?php
						
						$user_qry=mysqli_query($link, " SELECT * FROM `daily_account_close` WHERE `user`='$emp_id' ORDER BY `slno` DESC ");
					...
					<!--<button class="btn btn-success" onClick="view_all('daily_reports')">Daily Reports </button>->
...
<script>
	...
	{
		$("#loader").show();
		$.post("pages/daily_account_details_all_user_break.php",
		{
			type:typ,
			date1:$("#from").val(),
			date2:$("#to").val(),
	...
	function print_page(val,date1,date2,encounter,user_entry,pay_mode,account_break)
	{
		...
		if(val=="detail")
		{
			url="pages/detail_account_print_user_break.php?date1="+date1+"&date2="+date2+"&encounter="+encounter+"&user_entry="+user_entry+"&pay_mode="+pay_mode+"&EpMl="+user+"&account_break="+account_break;
		}
		if(val=="daily_reports")
		{
			url="pages/daily_reports_print.php?date1="+date1+"&date2="+date2+"&user_entry="+user_entry;
		}
		if(val=="summary")
		{
		...
		{
			url="pages/balance_received_print.php?date1="+date1+"&date2="+date2+"&encounter="+encounter+"&user_entry="+user_entry+"&pay_mode="+pay_mode+"&EpMl="+user+"&account_break="+account_break;
		}
		wind=window.open(url,'Window','scrollbars=1,menubar=1,toolbar=0,height=670,width=1000');
	}
