...
if($last_stock) // last stock of current date
{
	...
	mysqli_query($link,"UPDATE `inv_mainstock_details` SET `receive`='$add_qnt',`closing`='$closing' WHERE `branch_id`='$branch_id' AND `item_id`='$itm' AND `batch_no`='$bch' AND `date`='$date'");
	$txt.="\nUPDATE `inv_mainstock_details` SET `receive`='$add_qnt',`closing`='$closing' WHERE `branch_id`='$branch_id' AND `item_id`='$itm' AND `batch_no`='$bch' AND `date`='$date'";
}
else // last stock desc
{
	...
	mysqli_query($link,"INSERT INTO `inv_mainstock_details`(`branch_id`, `item_id`, `batch_no`, `opening`, `receive`, `issue`, `closing`, `date`) VALUES ('$branch_id','$itm','$bch','$opening','$vqnt','0','$closing','$date')");
	$txt.="\nINSERT INTO `inv_mainstock_details`(`branch_id`, `item_id`, `batch_no`, `opening`, `receive`, `issue`, `closing`, `date`) VALUES ('$branch_id','$itm','$bch','$opening','$vqnt','0','$closing','$date')";
}
