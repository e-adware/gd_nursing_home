...
$v=mysqli_fetch_assoc(mysqli_query($link,"SELECT * FROM `ph_challan_receipt_master` WHERE `order_no`='$ord'"));
?>
<!--header-->
<div id="content-header">
    <div class="header_div"> <span class="header">Challan Receive Items</span></div>
</div>
<!--End-header-->
<div class="container-fluid">
	<input type="hidden" id="ord_no" value="<?php echo base64_encode($ord);?>" />
	<div>
		<table class="table table-condensed entry_table">
			...
			<tr>
				<td>
					<input type="text" id="bill_no" placeholder="Challan No" class="imp" value="<?php echo $v['challan_no'];?>" disabled />
				</td>
				<td>
					
				</td>
				<td colspan="2">
					<input type="text" id="billdate" placeholder="Challan Date" class="imp" value="<?php echo $v['bill_date'];?>" disabled />
				</td>
				<td colspan="3">
	...
	<table class="table table-condensed entry_table">
		<tr>
			...
			<th>Discount</th>
			<td>
				<input type="text" id="discount" class="imp text_sm" onkeyup="chk_dec(this,event);calulate_discount(this.value,event)" disabled placeholder="Discount" />
			</td>
			<th>GST</th>
			<td>
			...
			<th>Adjust</th>
			<td>
				<input type="text" id="adjust" class="imp text_sm" onkeyup="chk_dec(this,event);adjust(this,event)" value="<?php echo $v['adjust_amt'];?>" placeholder="Adjust Amount" />
			</td>
			<th>Net Amount</th>
			<td>
...
function save_data_final()
{
	...
	else if($("#adjust").hasClass('err')==true)
	{
		$("#adjust").focus();
	}
	else
	{
		...
		function(data,status)
		{
			...
			for(var i=0; i<len; i++)
			{
				$(".tr_vals:eq("+i+")").find('td:eq(12)').html('<i class="icon-ok icon-large text-success"></i>');
			}
		})
	}
}
