<!--header-->
<div id="content-header">
    <div class="header_div"> <span class="header"> <?php echo $menu_info["par_name"]; ?></span></div>
</div>
<!--End-header-->
<div class="container-fluid">
	<div class="span5" style="margin-left: 0;">
		<table class="table table-bordered table-condensed">
			
			<tr>
				 <td>Select Category <input type="text" id="id" style="display:none;" /></td>
				<td>
					<select id="category_id" onChange="load_sub_category_list()" autofocus>
						<option value="0">--Select--</option>
						<?php
						$q=mysqli_query($link," SELECT * FROM `stock_category_master` ORDER BY `category_name` ");
						while($r=mysqli_fetch_array($q))
						{
						?>
						<option value="<?php echo $r['category_id'];?>"><?php echo $r['category_name'];?></option>
						<?php
						}
						?>
					</select>
				</td>
			</tr>
			<tr style="display:none;">
				<td>Sub Category ID</td>
				<td>
					<input type="text" id="sub_category_id" value="0" readonly="readonly" class="span3" />
				</td>
			</tr>
			
			<tr>
				<td>Sub Category Name</td>
				<td><input type="text" id="sub_category_name" class="span3" placeholder="sub category" autocomplete="off" autofocus /></td>
			</tr>
			
			<tr>
				<td colspan="2" style="text-align:center;">
					<input type="button" id="sav" class="btn btn-info" onclick="save()" value="Save" />
					<input type="button" id="" class="btn btn-danger" onclick="reset()" value="Reset" />
					<input type="button" id="" class="btn btn-success" onclick="print('pages/stock_sub_category_print.php')" value="View" />
				</td>
			</tr>
		</table>
	</div>
	<div class="span6">
		<b>Search</b> <input type="text" id="srch" placeholder="Search" onkeyup="search(this.value)">
		<div id="load_data" style="max-height:400px;overflow-y:scroll;">
		
		</div>
	</div>
</div>
<script>
	$(document).ready(function()
	{
		load_sub_category_list($("#category_id").val());
		$("#sub_category_name").keyup(function(e)
		{
			if(e.keyCode==13 && $(this).val()!="")
			{
				$("#sav").focus();
			}
		});
	});
	function reset()
	{
		$("#category_id").val(0).focus();
		$("#sub_category_id").val(0);
		$("#sub_category_name").val("");
		
		$("#sav").val("Save");
	}
	function load_sub_category_list()
	{
		$.post("pages/stock_sub_category_master_data.php",
		{
			type:"load_sub_category_list",
			user:$("#user").text().trim(),
			lavel_id:$("#lavel_id").val(),
			category_id:$("#category_id").val(),
		},
		function(data,status)
		{
			$("#load_data").html(data);
		})
	}
	function load_sub_category_detail(sub_category_id)
	{
		$.post("pages/stock_sub_category_master_data.php",
		{
			type:"load_sub_category_detail",
			sub_category_id:sub_category_id,
		},
		function(data,status)
		{
			var str=data.split("@#@");		
			$("#category_id").val(str[0]);
			$("#sub_category_id").val(str[1]);
			$("#sub_category_name").val(str[2]).focus();
			
			$("#sav").val("Update");
		})
	}
	function save()
	{
		$.post("pages/stock_sub_category_master_data.php",
		{
			type:"save_sub_category",
			category_id:$("#category_id").val(),
			sub_category_id:$("#sub_category_id").val(),
			sub_category_name:$("#sub_category_name").val(),
		},
		function(data,status)
		{
			bootbox.dialog({ message: data});
			setTimeout(function(){
				bootbox.hideAll();
				$("#sub_category_id").val(0);
				$("#sub_category_name").val("");
				load_sub_category_list();
			},500);
		})
	}
	function delete_sub_category(sub_category_id)
	{
		bootbox.dialog({
			//title: "Patient Re-visit ?",
			message: "<h5>Are you sure want to delete ?</h5>",
			buttons: {
				cancel: {
					label: '<i class="icon-remove"></i> Cancel',
					className: "btn btn-inverse",
					callback: function() {
					  bootbox.hideAll();
					}
				},
				confirm: {
					label: '<i class="icon-ok"></i> Delete',
					className: "btn btn-danger",
					callback: function() {
						$.post("pages/stock_sub_category_master_data.php",
						{
							type:"delete_sub_category",
							sub_category_id:sub_category_id,
						},
						function(data,status)
						{
							bootbox.dialog({ message: data});
							setTimeout(function(){
								bootbox.hideAll();
								load_sub_category_list();
							},500);
						})
					}
				}
			}
		});
	}
	function search(inputVal)
	{
		var table = $('#tblData');
		table.find('tr').each(function(index, row)
		{
			var allCells = $(row).find('td');
			if(allCells.length > 0)
			{
				var found = false;
				allCells.each(function(index, td)
				{
						var regExp = new RegExp(inputVal, 'i');
						if(regExp.test($(td).text()))
						{
								found = true;
								return false;
						}
				});
				if(found == true)
				{
						$("#no_record").text("");
						$(row).show();
				}else{
						$(row).hide();
						var n = $('tr:visible').length;
						if(n==1)
						{
								$("#no_record").text("No matching records found");
						}else
						{
								$("#no_record").text("");
						}
				}
				//if(found == true)$(row).show();else $(row).hide();
			}
		});
	}
	function print(url)
	{
		var category_id=$("#category_id").val();
		
		var user=$("#user").text().trim();
		url=url+"?category_id="+category_id+"&user="+user;
		window.open(url,'Window','scrollbars=1,menubar=1,toolbar=0,height=670,width=1000');
	}
</script>
<style>
	.nm:hover
	{
		color:#0000ff;
		cursor:pointer;
	}
</style>
