...
<title>IPD Patient Item Return</title>
...
function load_unpaidbill()
{
	$.post("pages/inv_load_unaprv_indnt.php",
	  {
		  type:"ph_ipd_item_return",
		  orderid:$("#txtcid").val(),
	  },
	  function(data,status)
	  {
		  $("#results").css({'height':'650px' });
				  
		 $("#results").html(data);
		 $("#myModal").animate({'top':'5%','left':'35%',"width":"1100px","height":"650px",'margin':'auto'},"slow");
...
function data_saved()
{
	...
	if(jj==1)
	{
		...
		for(var i=0;i<chk.length;i++)
		{
			...
			blno=blno+"@#"+$(chk[i]).val()+"%%"+$("#txtnwpaid_"+$(chk[i]).attr("id")+"").val()+"%%"+$("#txtbatchno_"+$(chk[i]).attr("id")+"").val()+"%%"+$("#txtitmid"+$(chk[i]).attr("id")+"").val();
...
<div id="content-header">
    <div class="header_div"> <span class="header">IPD Patient Medicine Return</span></div>
</div>
<!--End-header-->
<div class="container-fluid">
			...
			<div  style="margin-left:0px;">
					...
					<tr>
						...
						<td ></td>
						<td >  
						<input type="text" name="txtcid" id="txtcid" size="10" value="" readonly="readonly" class="imp intext"/><input type="text" name="txtsbstrid" id="txtsbstrid" size="15" value="" readonly="readonly" class="imp intext"/> Date&nbsp; <input type="text" name="txtdate" id="txtdate" value="<?php echo date('Y-m-d');?>" onfocus="jsdate('txtdate')" readonly />
						</td>
					</tr>
					</table>
			</div>
