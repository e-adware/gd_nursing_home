...
if($type==1)
{
	...
	$balance_amount=$tot_serv_amt-$settle_amount;
	
	if($emp_info['edit_payment']==0)
	{
	...
	$final_payment_slno_qry=mysqli_query($link," SELECT `pay_id` FROM `payment_detail_all` WHERE patient_id='$uhid' and opd_id='$ipd' and payment_type='Final' ");
	...
	if($final_payment_num>0)
	{
		...
		$f_slno=$final_payment_slno['pay_id'];
		$f_pay=1;
	}
	// Discharge Patient
...
?>
	...
	<table class="table">
		...
		<tr>
			...
			<th><?php echo number_format($already_paid,2); ?></th>
			<th><?php echo number_format($already_discount,2); ?></th>
			<th><?php echo number_format(($tot_serv_amt-$already_paid-$already_discount+$already_refund),2); ?></th>
		<?php if($already_refund>0){ ?>
			<th><?php echo number_format(($already_refund),2); ?></th>
		<?php } ?>
		</tr>
	</table>
	<table class="table">
	...
	<?php
	$i=1;	
	$qry=mysqli_query($link,"select * from payment_detail_all where patient_id='$uhid' and opd_id='$ipd' and payment_mode!='Credit' order by pay_id asc");
	$payment_det_num=mysqli_num_rows($qry);
	while($q=mysqli_fetch_array($qry))
	{
		$pay_mode_type=mysqli_fetch_array(mysqli_query($link, "SELECT `operation` FROM `payment_mode_master` WHERE `p_mode_name`='$q[payment_mode]'"));
		
		if($dis_none=="")
		{
			$discharge_num=mysqli_num_rows(mysqli_query($link," SELECT * FROM `ipd_pat_bed_details` WHERE `patient_id`='$uhid' AND `ipd_id`='$ipd' "));
			if($discharge_num>0)
			{
				if($f_pay==0)
				{
				...
				{
					if($f_slno==$q["pay_id"]){ $en_final=""; }else{ $en_final="display:none;"; }
				}
			}else
			{
				$dis_none="display:none;";
		...
		if($q["payment_mode"]=="Credit")
		{
			$display_paid_amount=$q["balance_amount"];
		}
		else
		{
			$display_paid_amount=$q["amount"];
		}
		if($q["payment_type"]=="Refund")
		{
			$display_paid_amount=$q["refund_amount"];
		}
?>
		<tr>
			<td><?php echo $i; ?></td>
			<td><?php echo $q["transaction_no"]; ?></td>
			<td><?php echo $display_paid_amount; ?></td>
			<td><?php echo $q["payment_type"]; ?></td>
			<td>
				<select class="span1" id="ipd_payment_mode_trans<?php echo $q["pay_id"]; ?>" onchange="payment_mode_change_trans('<?php echo $q["pay_id"]; ?>')">
				<?php
					...
					{
						if($pay_mode_master["p_mode_name"]==$q["payment_mode"]){ $sel="selected"; }else{ $sel=""; }
						echo "<option value='$pay_mode_master[p_mode_name]' $sel>$pay_mode_master[p_mode_name]</option>";
					}
				?>
				</select>
			</td>
			<td><?php echo convert_date($q[date])."/".convert_time($q[time]); ?></td>
			<td>
				<?php echo $user["name"]; ?>
				<button class="btn btn-print btn-mini" style="float:right;" onclick="print_transaction('<?php echo $q["pay_id"]; ?>')"><i class="icon-print"></i></button>
			<?php
				if($payment_det_num==$i && $emp_info["edit_payment"]==1){
			?>
					<button class="btn btn-delete btn-mini" style="float:right; <?php echo $dis_none." ".$en_final; ?>" onclick="delete_receipt('<?php echo $q["pay_id"]; ?>')" ><i class="icon-remove"></i></button>
			<?php
					}
			?>
			</td>
...
<?php
		//echo "<tr><td>$i</td><td>$q[transaction_no]</td><td>$display_paid_amount</td><td>$q[payment_type]</td><td>$q[payment_mode]</td><td>".convert_date($q[date])."/".convert_time($q[time])."</td><td>$user[name]</td><td> <input type='button' value='Print' class='btn btn-info' onclick='print_transaction(\"$q[pay_id]\")'> <input type='button' value='Edit' class='btn btn-warning' onclick='edit_ipd_paymentmode(\"$q[pay_id]\")' $edit_ipd_style> <input type='button' value='Cancel Payment' class='btn btn-danger' onclick='cancel_payment_receipt(\"$q[pay_id]\")'/ $dis_none $en_final> </td></tr>";
		$i++;
...
<?php
	...
	$final_bill=mysqli_query($link,"select * from payment_detail_all where patient_id='$uhid' and opd_id='$ipd' and payment_type='Final'");
...
?>
		<table class="table">
			<tr>
				<th>Total Bill Amount</th><th> : <span id="pat_total"><?php echo number_format($tot_serv_amt,2);?></span></th>
			</tr>
			<tr>
				<th>Discount</th><th>: <span id="pat_disc"><?php echo $already_discount;?></span></th>
			</tr>
			<tr>
				<th>Paid</th><th>: <span id="pay_advance"><?php echo $already_paid;?></span> 
				<input type='hidden' id='bill_id' value="<?php echo $det["transaction_no"];?>"/></th>
			</tr>
			<tr>
				<th>Balance</th><th>: <span id="pat_balance"><?php echo $balance_amount;?></span></th>
			</tr>
	<?php
		if($already_refund>0)
		{
	?>
			<tr>
				<th>Refunded</th><th>: <span id="pat_refund_amt"><?php echo $already_refund;?></span></th>
			</tr>
	<?php
		}
	?>
			<tr>
				<th>Payment Mode</th>
				<th>: <?php echo $det["payment_mode"];?> </th>
			</tr>
			<tr>
				<td colspan="2" style="text-align:center">
					...
					<input type="button" value="Print Receipt" class="btn btn-info" onclick="print_transaction('<?php echo $det["pay_id"];?>',100)"/>
	...
	<?php
		...
		if($balance<0)
		{
			$discount_facility="display:none;";
		}
		$discount_permission="";
		if($emp_info["discount_permission"]==0)
		{
			$discount_permission="readonly";
		}
	?>
			<tr>
				<th>Advance Paid</th><th> : <span id="pat_advance"><?php echo $already_paid;?></span></th>
			</tr>
			<tr>
		...
		?>
			<tr style="<?php echo $discount_facility; ?>">
				<th>Discount</th><th><input type="text" id="pat_disc" onkeyup="pat_discount(this)" <?php echo $discount_permission; ?> /></th>
			</tr>
		...
		?>
			<tr>
				<th>Now Pay</th>
				<th>
					<input type="text" id="pay_advance" onkeyup="pat_advance(this)" value="<?php echo $now_pay_vall;?>" <?php echo $now_pay_readonly; ?> /> 
		...
		<?php } ?>
			<tr>
				<th>Payment Mode</th>
				<th>
					<select id="p_mode" onchange="pay_mode_change_fn_bal()">
					<?php
						$pay_mode_qry=mysqli_query($link, " SELECT `p_mode_id`,`p_mode_name` FROM `payment_mode_master` WHERE `status`=0 $operation_str ORDER BY `sequence` ");
			...
			<tr id="balance_tr" style="display:none;">
				<th>Balance Reason</th><th><input type="text" id="balance_reason_dischrg" /></th>
			</tr>
	...
	<?php
		...
		$adv_pay_num=mysqli_num_rows(mysqli_query($link," SELECT * FROM `payment_detail_all` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' AND `payment_type`='Advance' "));
		if($adv_pay_num==0)
		{
			if($dis_req_num==0)
			{
	?>
		<!--<button class="btn btn-danger" onclick="cancel_casual()" <?php echo $dis_none; ?> >Cancel Patient</button>New-->
	<?php } }
		if($process==1)
		{
	?>
	...
	?>
		...
		<?php }else{
			...
			$doc_pay_num=mysqli_num_rows(mysqli_query($link," SELECT * FROM `payment_settlement_doc` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' "));
			if($doc_pay_num==0)
			{
		?>
...
else if($type==2)
{
	...
	if($val=="Save")
	{
		$serv_tot=mysqli_fetch_array(mysqli_query($link,"select ifnull(sum(amount),0) as tot from ipd_pat_service_details where patient_id='$uhid' and ipd_id='$ipd'"));
		...
		$balance=$tot_amount-$settle_amount-$discount_amount-$refund_amount-$tax_amount-$pay_ad;
		$balance_reason="";
		
		$bill_no=generate_bill_no_new($bill_name,$p_type_id);
		
		//~ if(mysqli_query($link,"INSERT INTO `ipd_advance_payment_details`(patient_id, ipd_id, bill_no, tot_amount, discount, amount, balance, refund, pay_type, pay_mode, reference_no, time, date, user) VALUES ('$uhid','$ipd','$bill_no','$tot_amount','0','$pay_ad','$bal','0','Advance','$mode','$reference_no','$time','$date','$user')"))
		
		if(mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$tot_amount','$already_paid','$pay_ad','$discount_amount','$discount_reason','$refund_amount','$refund_reason','$tax_amount','$tax_reason','$balance','$balance_reason','Advance','$mode','$cheque_ref_no','$user','$date','$time','$p_type_id') "))
		{
			echo $bill_no;
		}
...
else if($type=="final_2")
{
	...
	$discount_reason=mysqli_real_escape_string($link,$_POST['dis_reason']);
	$cheque_ref_no=mysqli_real_escape_string($link,$_POST['reference_no']);
	$balance_reason=mysqli_real_escape_string($link,$_POST['balance_reason']);
	...
	$balance=$tot_amount-$settle_amount-$discount_amount-$pay;
	if($balance<0)
	{
		$refund_amount=abs($balance);
		$tax_reason="Advance amount is greater than bill amount";
	}
	if($refund_amount>0)
	{
		$payment_type="Refund";
	...
	if($val=="Save")
	{
		if($pay>0 && $balance>0)
		{
			...
			if($pay>0)
			{
				$chk_final=mysqli_fetch_array(mysqli_query($link, " SELECT * FROM `payment_detail_all` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' AND `payment_type`='Final' AND `payment_mode`='$payment_mode' "));
				
				if(!$chk_final)
				{
					$bill_no=generate_bill_no_new($bill_name,$p_type_id);
					
					mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$tot_amount','$already_paid','$pay','$discount_amount','$discount_reason','$refund_amount','$refund_reason','$tax_amount','$tax_reason','$balance','$balance_reason','Final','$payment_mode','$cheque_ref_no','$user','$date','$time','$p_type_id') ");
		...
		else
		{
			if($refund_amount>0)
			{
				...
				mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$tot_amount','$already_paid','0','0','','0','','0','','0','','Final','Cash','','$user','$date','$time','$p_type_id') ");
				
				$bill_no=generate_bill_no_new($bill_name,$p_type_id);
				
				mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$tot_amount','$already_paid','$pay','$discount_amount','$discount_reason','$refund_amount','$refund_reason','$tax_amount','$tax_reason','$balance','$balance_reason','Refund','$payment_mode','$cheque_ref_no','$user','$date','$time','$p_type_id') ");
			}
			else
			{
				$bill_no=generate_bill_no_new($bill_name,$p_type_id);
		...
		while($pay_mode_master=mysqli_fetch_array($pay_mode_master_qry))
		{
			$cash_final_pay_qry=mysqli_query($link," SELECT * FROM `payment_detail_all` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' AND `payment_type`='Final' AND `payment_mode`='$pay_mode_master[p_mode_name]' ");
			...
			if($cash_final_pay_num>1)
			{
				...
				while($cash_final_pay_val=mysqli_fetch_array($cash_final_pay_qry))
				{
					if($h>1)
					{
						...
						{
							mysqli_query($link," DELETE FROM `payment_detail_all` WHERE `pay_id`='$cash_final_pay_val[pay_id]' ");
						}
					}
					$h++;
				}
...
else if($type==110)
{
	...
	if($delivery_check)
	{
		$dischr=mysqli_num_rows(mysqli_query($link,"select * from payment_detail_all where patient_id='$delivery_check[patient_id]' and opd_id='$delivery_check[ipd_id]' and `payment_type`='Final'"));
	}else
	{
		$dischr=mysqli_num_rows(mysqli_query($link,"select * from payment_detail_all where patient_id='$uhid' and opd_id='$ipd' and `payment_type`='Final'"));
	}
...
else if($type==112)
{
	...
	if(!$bed_idd){ $bed_idd=0; }
	
	mysqli_query($link,"insert into ipd_pat_service_details(patient_id,ipd_id,group_id,service_id,service_text,ser_quantity,rate,amount,days,user,time,date,bed_id, doc_id, ref_id) values('$uhid','$ipd','$group','$serv','$serv_text','$ser_quantity','$rate','$amount','$days','$user','$time','$ser_entry_date','$bed_idd','$consultantdoctorid','0')");
	
	if($group==141 && $pat_reg["type"]==3) // IPD
	{
		...
		while($ot_ch=mysqli_fetch_array($other_charge))
		{
			...
			mysqli_query($link,"INSERT INTO `ipd_pat_service_details`(`patient_id`, `ipd_id`, `group_id`, `service_id`, `service_text`, `ser_quantity`, `rate`, `amount`, `days`, `user`, `time`, `date`, `bed_id`, `doc_id`, `ref_id`) VALUES ('$uhid','$ipd','$charge[group_id]','$ot_ch[charge_id]','$charge[charge_name]','1','$charge[amount]','$charge[amount]','1','$user','$time','$ser_entry_date','$bed_idd','$consultantdoctorid','0')");
		}
		...
		//~ mysqli_query($link,"INSERT INTO `ipd_pat_service_details`(`patient_id`, `ipd_id`, `group_id`, `service_id`, `service_text`, `ser_quantity`, `rate`, `amount`, `days`, `user`, `time`, `date`, `bed_id`, `doc_id`, `ref_id`) VALUES ('$uhid','$ipd','173','1354','$charge_master_val[charge_name]','1','$charge_amount','$charge_amount','1','$_SESSION[emp_id]','$pat_reg[time]','$pat_reg[date]','$first_bed[bed_id]','$consultantdoctorid','0')");
	...
	{
		mysqli_query($link," INSERT INTO `doctor_service_done`(`patient_id`, `ipd_id`, `service_id`, `consultantdoctorid`, `user`, `date`, `time`, `rel_slno`, `schedule_id`) VALUES ('$uhid','$ipd','$serv','$consultantdoctorid','$user','$ser_entry_date','$time','$rel_slno','0') ");
	}
...
<?php
	...
	$edit_facility=0;
	if($emp_info["edit_payment"]=="1" && $dischr==0)
	{
		$edit_facility=1;
	}
			...
			<div class="accordion-body collapse" id="collapse<?php echo $dictinct_group["group_id"]; ?>_ser" style="height:0px;max-height:450px;overflow-y:scroll;">
				<div class="widget-content hidden_div_ser" id="cl<?php echo $dictinct_group["group_id"]; ?>_ser" style="display:none;">
					...
					<?php
						...
						while($q=mysqli_fetch_array($qry))
						{
							if(!$dischr && ($dictinct_group["group_id"]==104 || $dictinct_group["group_id"]==150  || $dictinct_group["group_id"]==151))
							{
								$test_check_num=mysqli_num_rows(mysqli_query($link, "SELECT `slno` FROM `patient_test_details` WHERE `slno`='$q[test_slno]'"));
								if($test_check_num==0)
								{
									mysqli_query($link, "DELETE FROM `ipd_pat_service_details` WHERE `slno`='$q[slno]'");
								}
							}
							...
							if($edit_facility==1)
							{
								$onclk="onclick=\"load_edit('$q[slno]')\"";
							}
							...
							echo "<tr $onclk style='cursor:pointer'><td>$i</td><td>$q[service_text]</td><td>$tot_days</td><td>$q[amount]</td><td>".convert_time($q["time"])."</td><td>".convert_date_g($q["date"])."</td></tr>";
...
else if($type==114)
{
	...
	$delivery_check=mysqli_fetch_array(mysqli_query($link," SELECT * FROM `ipd_pat_delivery_det` WHERE baby_uhid='$det[patient_id]' and baby_ipd_id='$det[opd_id]' "));
	if($delivery_check)
	{
		$dischr=mysqli_num_rows(mysqli_query($link,"select * from payment_detail_all where patient_id='$delivery_check[patient_id]' and opd_id='$delivery_check[ipd_id]' and `payment_type`='Final'"));
	}else
	{
		$dischr=mysqli_num_rows(mysqli_query($link,"select * from payment_detail_all where patient_id='$det[patient_id]' and opd_id='$det[opd_id]' and `payment_type`='Final'"));
	}
...
?>
	...
	//~ if($rel_slno_num>0)
	if($det["doc_id"]>0 && $det["doc_id"]!=99999)
	{
	?>
		<tr>
			<th>Doctor</th>
			<th>
				...
				<?php
					...
					while($con_doc=mysqli_fetch_array($con_doc_qry))
					{
						if($det["doc_id"]==$con_doc["consultantdoctorid"]){ $sel_doc="selected"; }else{ $sel_doc=""; }
	...
	<tr>
		<td colspan="2" style="text-align:center">
		<?php if($tot_num==0 && $edit_facility==1){ ?>
...
else if($type==115)
{
	...
	if($typ==1)
	{
		...
		if($ipd_service)
		{
			...
			//mysqli_query($link," INSERT INTO `ipd_pat_service_details_edit`(`patient_id`, `ipd_id`, `group_id`, `service_id`, `service_text`, `ser_quantity`, `rate`, `amount`, `days`, `user`, `time`, `date`, `counter`, `bed_id`, `doc_id`, `ref_id`) VALUES ('$ipd_service[patient_id]','$ipd_service[ipd_id]','$ipd_service[group_id]','$ipd_service[service_id]','$ipd_service[service_text]','$ipd_service[ser_quantity]','$ipd_service[rate]','$ipd_service[amount]','$ipd_service[days]','$ipd_service[user]','$ipd_service[time]','$ipd_service[date]','$counter_num','$ipd_service[bed_id]','$ipd_service[doc_id]','$ipd_service[ref_id]') ");
			
			
			mysqli_query($link, "INSERT INTO `ipd_pat_service_details_edit`(`slno`, `patient_id`, `ipd_id`, `group_id`, `service_id`, `service_text`, `ser_quantity`, `rate`, `amount`, `days`, `user`, `time`, `date`, `bed_id`, `doc_id`, `ref_id`, `test_slno`, `del_user`, `del_date`, `del_time`) SELECT *,'$c_user','$now_date','$now_time' FROM `ipd_pat_service_details` WHERE `patient_id`='$ipd_service[patient_id]' AND `ipd_id`='$ipd_service[ipd_id]' AND `slno`='$slno' ");
		}
		...
		mysqli_query($link,"update ipd_pat_service_details set service_id='$serv',service_text='$serv_text',ser_quantity='$ser_quantity',rate='$rate',days='$days',amount='$amount',time='$time',date='$date',`doc_id`='$consultantdoctorid' where slno='$slno'");
	...
	else if($typ==2)
	{
		...
		//~ mysqli_query($link,"insert into ipd_pat_service_delete(patient_id,ipd_id,group_id,service_id,service_text,ser_quantity,rate,amount,days,user,time,date,bed_id,doc_id,ref_id) values('$ipd_service[patient_id]','$ipd_service[ipd_id]','$ipd_service[group_id]','$ipd_service[service_id]','$ipd_service[service_text]','$ipd_service[ser_quantity]','$ipd_service[rate]','$ipd_service[amount]','$ipd_service[days]','$user','$time','$date','$ipd_service[bed_id]','$ipd_service[doc_id]','$ipd_service[ref_id]')");
		...
		//~ {
			...
			//~ mysqli_query($link,"insert into  ipd_pat_service_delete(patient_id,ipd_id,group_id,service_id,service_text,ser_quantity,rate,amount,days,user,time,date,bed_id,doc_id,ref_id) values('$ipd_service[patient_id]','$ipd_service[ipd_id]','$ch[group_id]','$other_service[charge_id]','$ch[charge_name]','$ipd_service[ser_quantity]','$ipd_service[rate]','$ipd_service[amount]','$ipd_service[days]','$user','$time','$date','$ipd_service[bed_id],'$ipd_service[doc_id],'$ipd_service[ref_id]')");
		...
		mysqli_query($link,"delete from patient_test_details where slno='$ipd_service[test_slno]'");
	...
	$pat_discharge=mysqli_fetch_array(mysqli_query($link,"SELECT * FROM `payment_detail_all` WHERE `patient_id`='$patient_id' AND `opd_id`='$ipd_id' AND payment_type='Final' "));
	if($pat_discharge)
	{
		// Manage Account
...
else if($type==117)
{
	...
	if($pat_bed_num==0)
	{
		...
		if($already_pat_bed_num==0)
		{
			...
			mysqli_query($link," UPDATE `payment_detail_all` SET `payment_type`='Advance',`discount_amount`='0',`refund_amount`='0',`tax_amount`='0',`balance_amount`='0' WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' AND `payment_type`='Final' ");
			
			mysqli_query($link," DELETE FROM `payment_detail_all` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' AND `payment_mode`='Credit' ");
...
else if($type=="1001_old")
{
	...
	$check_payment_entry=mysqli_fetch_array(mysqli_query($link,"select * from payment_detail_all where patient_id='$uhid' and opd_id='$ipd'"));
	...
	if($already_discount>0)
	{
		echo "<tr><th colspan='2' style='text-align:right'>Discount Amount</th><th>$already_discount</th>";
	}
	...
	echo number_format($already_paid,2);
	$tot_p=$already_paid;
	
	if($balance_amount>0 && $check_payment_entry)
	{
		echo "<tr><th colspan='2' style='text-align:right'>Balance</th><th>$balance_amount</th>";
	}
	
	if($already_refund>0)
	{
		echo "<tr><th colspan='2' style='text-align:right'>Refunded Amount</th><th>$already_refund</th>";
	}
	
	$bill=mysqli_fetch_array(mysqli_query($link,"select bill_no from payment_detail_all where patient_id='$uhid' and opd_id='$ipd' order by pay_id desc limit 1"));
	...
	{
		$final_payment_num=mysqli_num_rows(mysqli_query($link," SELECT * FROM `payment_detail_all` WHERE patient_id='$uhid' and opd_id='$ipd' and payment_type='Final' "));
		//$final_payment_cancel_num=mysqli_num_rows(mysqli_query($link," SELECT * FROM `payment_detail_all_cancel` WHERE patient_id='$uhid' and opd_id='$ipd' and payment_type='Final' "));
		if($final_payment_num>0)
		{
			$cancel_payment_btn="";
		}else
	...
	echo "<input type='hidden' value='$balance_amount' id='to_be_paid'/>";
	echo "<input type='hidden' value='$bill[transaction_no]' id='casual_bill_id'/>";
	echo "</th></tr>";
	
	//echo "<tr><th colspan='3' style='text-align:center'>";
	
	$payment_entry=mysqli_num_rows(mysqli_query($link," SELECT * FROM `payment_detail_all` WHERE patient_id='$uhid' and opd_id='$ipd' and payment_type='Final' "));
	...
	?>
		...
		<tr>
			...
			<td><input type="text" id="casual_to_pay" class="span2" value="<?php echo $balance_amount; ?>" disabled></td>
		</tr>
		
		<tr>
			<th colspan='2' style='text-align:right'>Cash</th>
			<td><input type="text" id="cash_pay" class="span2" value="<?php echo $balance_amount; ?>" onKeyup="cash_amt(this.value)" ></td>
		</tr>
		<tr>
	...
	<?php
		...
		//echo "<button class='btn btn-delete' onclick='cancel_casual()' $dis_none><i class='icon-remove'></i> Cancel Patient</button>";New
...
else if($type==5001)
{
	...
	$final_pay_num=mysqli_num_rows(mysqli_query($link," SELECT * FROM `payment_detail_all` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' AND `payment_type`='Final' "));
			...
			while($bd_lst=mysqli_fetch_array($bed_list))
			{
				...
				for($i=0;$i<$diff;$i++)
				{
					...
					if($chk_serv==0)
					{
						...
						// Bed Charge
						mysqli_query($link,"INSERT INTO `ipd_pat_service_details`(`patient_id`, `ipd_id`, `group_id`, `service_id`, `service_text`, `ser_quantity`, `rate`, `amount`, `days`, `user`, `time`, `date`, `bed_id`, `doc_id`, `ref_id`) VALUES ('$uhid','$ipd','$charge_det[group_id]','$bed_det[charge_id]','$charge_det[charge_name]','1','$charge_det[amount]','$charge_det[amount]','1','$_SESSION[emp_id]','$time','$n_date','$bd_lst[bed_id]','0','0')");
						...
						while($ot_ch=mysqli_fetch_array($other_charge))
						{
							...
							mysqli_query($link,"INSERT INTO `ipd_pat_service_details`(`patient_id`, `ipd_id`, `group_id`, `service_id`, `service_text`, `ser_quantity`, `rate`, `amount`, `days`, `user`, `time`, `date`, `bed_id`, `doc_id`, `ref_id`) VALUES ('$uhid','$ipd','$charge[group_id]','$ot_ch[charge_id]','$charge[charge_name]','1','$charge[amount]','$charge[amount]','1','$_SESSION[emp_id]','$time','$n_date','$bd_lst[bed_id]','0','0')");
						}
					}
				}
			}
	...
	if($delivery_check_val)
	{
		$final_serv_rel=mysqli_fetch_array(mysqli_query($link," SELECT * FROM `payment_detail_all` WHERE patient_id='$delivery_check_val[patient_id]' and opd_id='$delivery_check_val[ipd_id]' and payment_type='Final' "));
...
if($type=="payment_mode_change_each")
{
	//print_r($_POST);
...
if($type=="delete_payment_each")
{
	...
	if(mysqli_query($link, " DELETE FROM `payment_detail_all` WHERE `pay_id`='$pay_id' AND `patient_id`='$patient_id' AND `opd_id`='$opd_id' "))
	{
...
if($type==1001)
{
	$patient_id=$uhid=$_POST['uhid'];
	$opd_id=$ipd=$_POST['ipd'];
	$usr=$_POST['usr'];
	
	$pat_reg=mysqli_fetch_array(mysqli_query($link,"SELECT * FROM `uhid_and_opdid` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd'"));
	$p_type_id=$pat_reg["type"];
	
	$pat_reg_type=mysqli_fetch_array(mysqli_query($link,"SELECT `prefix` FROM `patient_type_master` WHERE `p_type_id`='$p_type_id'"));
	$prefix_name=$pat_reg_type["prefix"];
	...
	$chk_final_num=mysqli_num_rows(mysqli_query($link, " SELECT * FROM `payment_detail_all` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' AND `payment_type`='Final' "));
	
	$check_payment_num=mysqli_num_rows(mysqli_query($link,"select * from payment_detail_all where patient_id='$uhid' and opd_id='$ipd'"));
	
	$save_element_style="";
	if($check_payment_num==0)
	{
		$save_type_str="Save";
	...
	// Total
	$bill_amount=$tot_serv_amt=$tot_serv_amt1+$tot_serv_amt2+$baby_serv_tot+$ot_total+$baby_ot_total;
	...
	$disount_amount=$pay_det["dis_amount"];
	$tax_amount=$pay_det["tax"];
	$refund_amount=$pay_det["ref_amount"];
	
	$settle_amount=$paid_amount+$disount_amount+$tax_amount-$refund_amount;
	
	$due_amount=$bill_amount-$settle_amount;
	
	$due_amount_str=number_format($due_amount);
	
	if($bill_amount==0)
	{
		$payment_type_advance_sel="selected";
		$payment_type_final_sel="";
	}
	else
	{
		$payment_type_advance_sel="";
		$payment_type_final_sel="selected";
	}
?>
	<div id="payment_list" class="payment_class">
		<table class="table">
			...
			<tr>
				...
				<th><?php echo number_format($disount_amount,2); ?></th>
				<th><?php echo number_format(($due_amount),2); ?></th>
			<?php if($refund_amount>0){ ?>
				<th><?php echo number_format(($refund_amount),2); ?></th>
			<?php } ?>
			</tr>
		</table>
		...
		?>
				...
				<?php
					if($payment_det_num==1 && $payment_det["payment_type"]=="Final")
					{
		...
		<div id="res_payment_div">
			<table class="table table-condensed">
				...
				<tr style="<?php echo $save_element_style; ?>">
					<th>Discount Amount</th>
					<td>
						<span id="opd_disount_amount_str"><?php echo $disount_amount; ?></span>
						<input type="hidden" id="opd_disount_amount" value="<?php echo $disount_amount; ?>">
					</td>
				</tr>
				<tr style="<?php echo $save_element_style; ?>">
					<th>Paid Amount</th>
		...
		?>
				<tr style="<?php echo $save_element_style; ?>">
					...
					<td>
						<span id="opd_balance_amount_str"><?php echo $due_amount_str; ?></span>
				...
				<tr id="opd_now_pay_tr">
					<th>Now Pay</th>
					<td>
						<input type="text" class="span2 numericc" id="opd_now_pay" value="<?php echo $due_amount; ?>" onkeyup="opd_now_pay(event)" onpaste="return false;" ondrop="return false;" onfocus="$(this).select();">
					</td>
				</tr>
				...
				<tr id="opd_now_payment_type_tr">
					<th>Payment Type</th>
					<td>
						<select class="span2" id="opd_now_payment_type" onchange="opd_now_payment_type_change(this.value)" onkeyup="opd_now_payment_type_up(event)">
						<?php if($chk_final_num==0){ ?>
							<option value="Advance" <?php echo $payment_type_advance_sel; ?>>Advance</option>
						<?php } ?>
							<option value="Final" <?php echo $payment_type_final_sel; ?>>Final</option>
						</select>
					</td>
				</tr>
				...
				<tr id="save_tr">
					...
					<?php if($check_payment_num>0){ ?>
						
						<button class="btn btn-print" id="print_con_bill_btn" onClick="print_receipt_new('pages/ipd_bill_type_detail_casual_new.php?v=1')"><i class="icon-print"></i> Bill</button>
					<?php if($pat_reg["type"]==4){ ?>
						<button class="btn btn-print" id="opd_prescription_btn" onClick="print_receipt_new('pages/prescription_rpt_casu_new.php?v=0')"><i class="icon-print"></i> Prescription</button>
						
						<button class="btn btn-print print_assess lite" id="opd_assess_btn" onClick="print_receipt_new('pages/opd_assessment_new.php?v=0')"><i class="icon-print"></i> Assessment Form</button>
					<?php } ?>
					<?php } ?>
					</td>
				</tr>
...
if($type=="save_payment_other")
{
	//~ print_r($_POST);
	//~ exit();
	
	$uhid=$patient_id=mysqli_real_escape_string($link, $_POST["patient_id"]);
	$ipd=$opd_id=mysqli_real_escape_string($link, $_POST["opd_id"]);
	...
	$pay=mysqli_real_escape_string($link, $_POST["opd_now_pay"]);
	$payment_mode=mysqli_real_escape_string($link, $_POST["opd_now_payment_mode"]);
	$payment_type=mysqli_real_escape_string($link, $_POST["opd_now_payment_type"]);
	...
	if($payment_type=="Advance")
	{
		if($pay>0)
		{
			...
			mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$bill_amount','$already_paid','$pay','$discount_amount','$discount_reason','$refund_amount','$refund_reason','$tax_amount','$tax_reason','$balance_amount','$balance_reason','$payment_type','$payment_mode','$cheque_ref_no','$user','$date','$time','$p_type_id') ");
	...
	{
		$balance_amount=$bill_amount-$settle_amount-$discount_amount-$pay;
		
		if($balance_amount<0)
		{
			$refund_amount=abs($balance_amount);
			$refund_reason="Bill amound has beed reduced";
		...
		$chk_final=mysqli_fetch_array(mysqli_query($link, " SELECT * FROM `payment_detail_all` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' AND `payment_type`='Final' "));
		if(!$chk_final && $bill_amount>0)
		{
			if($pay>0 && $balance_amount>0)
			{
				// Two row insert- one --> Pay - two --> Credit(Balance)
				if($pay>0)
				{
					...
					if(!$chk_final)
					{
						...
						mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$bill_amount','$already_paid','$pay','$discount_amount','$discount_reason','$refund_amount','$refund_reason','$tax_amount','$tax_reason','$balance_amount','$balance_reason','Final','$payment_mode','$cheque_ref_no','$user','$date','$time','$p_type_id') ");
				...
				if($balance_amount>0)
				{
					...
					if(!$chk_final)
					{
						...
						if($balance_amount>0)
						{
							mysqli_query($link," INSERT INTO `ipd_discharge_balance_pat`(`patient_id`, `ipd_id`, `bal_amount`, `user`, `date`, `time`) VALUES ('$uhid','$ipd','$balance_amount','$user','$date','$time') ");
						}
					}
				}
			}
			else
			{
				if($refund_amount>0)
				{
					...
					mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$bill_amount','$already_paid','0','0','','0','','0','','0','','Final','Cash','','$user','$date','$time','$p_type_id') ");
					
					$bill_no=generate_bill_no_new($bill_name,$p_type_id);
					
					mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$bill_amount','$already_paid','$pay','$discount_amount','$discount_reason','$refund_amount','$refund_reason','$tax_amount','$tax_reason','$balance_amount','$balance_reason','Refund','$payment_mode','$cheque_ref_no','$user','$date','$time','$p_type_id') ");
				}
				else
				{
					$bill_no=generate_bill_no_new($bill_name,$p_type_id);
		...
		{
			if($pay>0 || $discount_amount>0 || $refund_amount>0)
			{
				if($refund_amount>0)
				{
					$bill_no=generate_bill_no_new($bill_name,$p_type_id);
				...
				else
				{
					...
					mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`, `encounter`) VALUES ('$uhid','$ipd','$bill_no','$bill_amount','$already_paid','$pay','$discount_amount','$discount_reason','$refund_amount','$refund_reason','$tax_amount','$tax_reason','$balance_amount','$balance_reason','Balance','$payment_mode','$cheque_ref_no','$user','$date','$time','$p_type_id') ");
				}
				
				mysqli_query($link," UPDATE `ipd_discharge_balance_pat` SET `bal_amount`='$balance_amount' WHERE `patient_id`='$uhid' and `ipd_id`='$ipd' ");
