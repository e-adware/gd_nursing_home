...
$top_line_break=4;
$doc_in_a_line=4;
$max_line_in_a_page=22;
$single_page_test_param_num=4;
$div_height="height: 490px;";
...
if($doc_view==1 && $tests=="") // Doctor Approval Page
{
	$all_test_qry=mysqli_query($link, "SELECT a.`testid` FROM `patient_test_details` a, `testmaster` b WHERE a.`testid`=b.`testid` AND b.`category_id`=1 AND a.`patient_id`='$uhid' AND `opd_id`='$opd_id' AND `ipd_id`='$ipd_id' AND `batch_no`='$batch_no' ORDER BY b.`type_id`,b.`testid` ASC");
	...
	while($report_doc=mysqli_fetch_array($report_doc_qry))
	{
		...
		?>
			<div class="container-fluid"> <!-- style="border: 2px solid #000;height: 1080px;" -->
				...
				<div class="row report_div" style="<?php echo $div_height; ?>">
					<table class="table table-condensed table-no-top-border report_table">
						<tr class="report_header">
							...
							<th class="test_result" style="text-align:center;">RESULTS</th>
					<?php
				...
				?>
						<tr>
							<th colspan="5" class="test_name no_top_border"><?php echo $test_info["testname"]; ?></th>
						</tr>
				<?php
							}
						}
		...
		?>
			...
			?>
				...
				<?php
							...
							$result_note=mysqli_fetch_array(mysqli_query($link,"SELECT `note` FROM `testresults_note` WHERE `patient_id`='$uhid' AND `opd_id`='$opd_id' AND `ipd_id`='$ipd_id' AND `batch_no`='$batch_no' AND `testid`='$testid'"));
							if($result_note["note"])
							{
								echo "<tr><td colspan='5'><br>N:B:- $result_note[note]</td></tr>";
							}
							...
							if($test_summary_text)
							{
								echo "<tr><td colspan='5'><br>$test_summary_text</td></tr>";
							}
						}
					}
				?>
