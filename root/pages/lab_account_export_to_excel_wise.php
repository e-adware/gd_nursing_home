...
<div class="container">
	...
	<p style="margin-top: 2%;"><b><?php echo $dept; ?></b><br><b>Lab Account from:</b> <?php echo convert_date($date1)." <b>to</b> ".convert_date($date2); ?></p>
	...
	<?php
		...
		while($pat_reg=mysqli_fetch_array($pat_reg_qry))
		{
			...
			$pat_rate_qry=mysqli_query($GLOBALS["___mysqli_ston"], " SELECT `test_rate` FROM `patient_test_details` WHERE `patient_id`='$pat_reg[patient_id]' and `opd_id`='$pat_reg[opd_id]' and `testid` in (SELECT `testid` FROM `testmaster` WHERE `category_id`='$val') ");
			$qqq="";
