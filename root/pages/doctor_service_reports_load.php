...
if($_POST["type"]=="doctor_service_report")
{
	$service_id=trim($_POST['service_id']);
	$consultantdoctorid=trim($_POST['consultantdoctorid']);
...
?>
	<p style="margin-top: 2%;"><b>Doctor Service Reports from:</b> <?php echo convert_date($date1)." <b>to</b> ".convert_date($date2); ?>
		...
		<button type="button" class="btn btn-info text-right" onclick="print_page('doctor_service','<?php echo $date1;?>','<?php echo $date2;?>','<?php echo $service_id;?>','<?php echo $consultantdoctorid;?>')" style="margin-right: 1%;"><i class="icon-print icon-large"></i> Print</button>
		
		<a class="btn btn-info text-right" href="pages/doctor_service_reports_excel.php?date1=<?php echo $date1;?>&date2=<?php echo $date2;?>&service_id=<?php echo $service_id;?>&consultantdoctorid=<?php echo $consultantdoctorid;?>" style="margin-right: 5px;"><i class="icon-file icon-large"></i>Excel</a>
	</p>
...
?>
	...
	?>
		<tr>
			...
			<td colspan="4"><?php echo $rupees_symbol.number_format($tot_bill_amout,2); ?></td>
		</tr>
	</table>
<?php
	}
