...
<?php
	...
	if($_GET["uhid"])
	{
		...
		$pat_emergency_det=mysqli_fetch_array(mysqli_query($link, " SELECT * FROM `emergency_patient_details` WHERE `patient_id`='$uhid' AND `opd_id`='$ipd' "));
		$mlc_style="style='display:none;'";
		if($pat_emergency_det["type_id"]==9)
		{
			$mlc_style="";
		}
?>
	<table id="padd_tbl" class="table">
		...
		<tr style="display:none;">
			...
			<td colspan="5">
				...
				<?php
					$pat_type_qry=mysqli_query($link, " SELECT * FROM `patient_type_master` WHERE `p_type_id`='$p_type_id'");
		...
		<tr>
			...
			<th>Patient Type</th>
			<td colspan="">
				<select class="span2" id="visit_type_id" onkeyup="visit_type_id(this.value,event)">
			<?php
				$visit_type_qry=mysqli_query($link, "SELECT `visit_type_id`, `visit_type_name` FROM `patient_visit_type_master` WHERE `p_type_id`='4' ORDER BY `visit_type_id`");
		...
		<tr>
			<th>Emergency Doctor</th>
			<td colspan="3">
				<!--<select class="span2" id="e_doc_id" onChange="doc_sel()" onKeyUp="doc_sel_Up(event)">-->
				<select class="span4" id="e_doc_id">
					<option value="0">Select</option>
				<?php
					$doc_qry=mysqli_query($link, " SELECT * FROM `consultant_doctor_master` WHERE `status`='0' ORDER BY `Name` ");
					
					while($doc=mysqli_fetch_array($doc_qry))
					{
						if($pat_emergency_det["consultantdoctorid"]==$doc["consultantdoctorid"]){ $doc_sel="selected"; }else{ $doc_sel=""; }
						echo "<option value='$doc[consultantdoctorid]' $doc_sel >$doc[Name]</option>";
					}
				?>
				</select>
			</td>
			...
			<td colspan="1">
				<!--<select class="span2" id="pat_visit_type" onChange="pat_visit_type_ch()" onKeyUp="pat_visit_type_up(event)">-->
				<select class="span" id="pat_visit_type">
					<option value="0">Select</option>
				<?php
					$dept_qry=mysqli_query($link, " SELECT `type_id`, `type_name` FROM `emergency_visit_type_master` ORDER BY `type_name` ");
					while($dept=mysqli_fetch_array($dept_qry))
					{
						if($pat_emergency_det["type_id"]==$dept["type_id"]){ $type_sel="selected"; }else{ $type_sel=""; }
						echo "<option value='$dept[type_id]' $type_sel >$dept[type_name]</option>";
					}
				?>
				</select>
			</td>
		</tr>
		<tr class="police_detail" <?php echo $mlc_style; ?>>
			<th>Police Station</th>
			<td>
				<input type="text" class="span2" id="police_detail_station" value="<?php echo $pat_emergency_det["police_station"]; ?>" onKeyup="police_detail_station_up(event,this.value,this.id)" list="police_detail_station_datalist">
				<datalist id="police_detail_station_datalist">
				<?php
					$data_datalist_qry=mysqli_query($link," SELECT DISTINCT `police_station` FROM `emergency_patient_details` WHERE `police_station`!='' ORDER BY `police_station` ");
					while($data_datalist=mysqli_fetch_array($data_datalist_qry))
					{
						echo "<option value='$data_datalist[police_station]'></option>";
					}
				?>
				</datalist>
			</td>
			<th>Investigation Officer</th>
			<td><input type="text" class="span2" id="police_detail_officer" onKeyup="police_detail_officer_up(event,this.value,this.id)" value="<?php echo $pat_emergency_det["officer"]; ?>"></td>
			<th>Case No</th>
			<td><input type="text" class="" id="police_detail_case_no" onKeyup="police_detail_case_no_up(event)" value="<?php echo $pat_emergency_det["case_no"]; ?>"></td>
		</tr>
		<tr class="police_detail" <?php echo $mlc_style; ?>>
			<th>Case Entry Date Time</th>
			<td>
				<input type="text" class="datepicker span2" id="police_detail_date" onKeyup="police_detail_date_up(event,this.value,this.id)" value="<?php echo $pat_emergency_det["case_date"]; ?>" readonly>
				<input type="text" class="timepicker span1" id="police_detail_time" onKeyup="police_detail_time_up(event)" value="<?php echo $pat_emergency_det["case_time"]; ?>" readonly>
			</td>
			<th>Case Description</th>
			<td colspan="3"><input type="text" class="" id="police_detail_desc" onKeyup="police_detail_desc_up(event,this.value,this.id)" style="width: 95%;" value="<?php echo $pat_emergency_det["description"]; ?>"></td>
		</tr>
		<tr>
	...
	<table id="padd_tbl" class="table table-condensed">
		...
		<tr>
			...
			<?php
				...
				while($visit_type=mysqli_fetch_array($visit_type_qry))
				{
					echo "<option value='$visit_type[visit_type_id]'>$visit_type[visit_type_name]</option>";
				}
			?>
				</select>
			</td>
		...
		<tr>
			...
			<td colspan="3">
				<select class="span4" id="e_doc_id">
					<option value="0">Select</option>
				<?php
					//$doc_qry=mysqli_query($link, " SELECT a.* FROM `consultant_doctor_master` a, `employee` b WHERE a.`emp_id`=b.`emp_id` AND b.`status`='0' AND a.`dept_id`=46 ORDER BY `Name` ");
					$doc_qry=mysqli_query($link, " SELECT * FROM `consultant_doctor_master` WHERE `status`='0'  ORDER BY `Name` ");
					while($doc=mysqli_fetch_array($doc_qry))
					{
						echo "<option value='$doc[consultantdoctorid]'>$doc[Name]</option>";
					}
				?>
				</select>
			</td>
			...
			<td colspan="1">
				...
				<?php
					...
					while($dept=mysqli_fetch_array($dept_qry))
					{
						if($dept["type_id"]==1){ $sel="selected"; }else{ $sel=""; }
						echo "<option value='$dept[type_id]' $sel>$dept[type_name]</option>";
					}
				?>
				</select>
			</td>
		</tr>
		<tr class="police_detail" style="display:none;">
			<th>Police Station</th>
			<td>
				<input type="text" class="span2" id="police_detail_station" onKeyup="police_detail_station_up(event,this.value,this.id)" list="police_detail_station_datalist">
				<datalist id="police_detail_station_datalist">
				<?php
					$data_datalist_qry=mysqli_query($link," SELECT DISTINCT `police_station` FROM `emergency_patient_details` WHERE `police_station`!='' ORDER BY `police_station` ");
					while($data_datalist=mysqli_fetch_array($data_datalist_qry))
			...
			<th>Investigation Officer</th>
			<td><input type="text" class="span2" id="police_detail_officer" onKeyup="police_detail_officer_up(event,this.value,this.id)"></td>
			<th>Case No</th>
			<td><input type="text" class="" id="police_detail_case_no" onKeyup="police_detail_case_no_up(event)"></td>
		</tr>
		<tr class="police_detail" style="display:none;">
			<th>Case Entry Date Time</th>
			<td>
				<input type="text" class="datepicker span2" id="police_detail_date" onKeyup="police_detail_date_up(event,this.value,this.id)" readonly>
				<input type="text" class="timepicker span1" id="police_detail_time" onKeyup="police_detail_time_up(event)" readonly>
			</td>
			<th>Case Description</th>
			<td colspan="3"><input type="text" class="" id="police_detail_desc" onKeyup="police_detail_desc_up(event,this.value,this.id)" style="width: 92%;"></td>
		</tr>
		<tr style="display:none;">
			<th>Date</th>
			<td>
...
<script src="../jss/casualty_reg.js"></script>
...
<script>
	$(document).ready(function()
	{
		...
		$("#e_doc_id").select2({ theme: "classic" });
		$("#e_doc_id").on("select2:close",function()
		{
			if($("#e_doc_id").val()!="0")
			{
				setTimeout(function()
				{
					$("#pat_visit_type").select2("focus");
				},200);
			}
		});
	...
	function save(typ)
	{
		...
		else if($("#e_doc_id").val()=="0")
		{
			//$("#dept_id").css('border','1px solid #ff0000');
			//$("#e_doc_id").focus();
			$("#e_doc_id").select2("focus");
		}
		else if($("#pat_visit_type").val()=="0")
		{
			//$("#dept_id").css('border','1px solid #ff0000');
			//$("#e_doc_id").focus();
			$("#pat_visit_type").select2("focus");
		...
		else
		{
			...
			$.post("pages/casualty_reg_ajax.php",
			{
				...
				e_doc_id:$("#e_doc_id").val(),
				
				pat_visit_type:$("#pat_visit_type").val(),
				police_detail_station:$("#police_detail_station").val(),
				police_detail_officer:$("#police_detail_officer").val(),
				police_detail_case_no:$("#police_detail_case_no").val(),
				police_detail_date:$("#police_detail_date").val(),
				police_detail_time:$("#police_detail_time").val(),
				police_detail_desc:$("#police_detail_desc").val(),
			...
			function(data,status)
			{
				...
				if(vl[2]==0)
				{
					...
					setTimeout(function()
					{
						if(vl[3]==4)
						{
							window.location="processing.php?param=85&uhid="+vl[0]+"&ipd="+vl[1];
						}
						if(vl[3]==8)
						{
							window.location="processing.php?param=119&uhid="+vl[0]+"&ipd="+vl[1];
						}
					}, 1000);
				}else
				{
