				...
				<div class="col-md-12">
					...
					<table class="table borderless bordert-top-bottom">
						...
						<tr>
							...
							<td>: <?php echo convert_date($completion_date)."/".convert_time($completion_time);?></td>
						</tr>
						<!--<tr>
							<td>Ref. By</td>
				...
				<div class="row">
					<div class="col-xs-12">
						<?php
							...
							while($dd=mysqli_fetch_array($doc))
							{
							if($doc_nm==1)
							{ ?>
						<div class="row">
					...
					<div class="col-xs-2 nabl-logo">
						<!--<img src="../../images/NABL_LOGO_PRINT2.jpg"/>
					</div>
					
					<?php	
						
						$tech_n1=explode(",",$tech_n);
						$tch=array_unique($tech_n1);
						...
						foreach($l_us as $ls)
						{
							if($ls)
							{
								...
								{
									$user_n=mysqli_fetch_array(mysqli_query($link, "select * from Employee where ID='$ls'"));
									$usr.=$user_n[Name].",";
								}
							}
						}
					...
					<div class="col-xs-4" style="font-size:10px;">
						Data entry by: <?php echo rtrim($usr,",");?>
					</div>
					<div class="col-xs-4" style="font-size:10px;text-align:center;width:345px">
						Checked by : <?php echo rtrim($techn, ",");?>
					</div>
					<div class="col-xs-4" style="font-size:10px;text-align:right;width:120px;">
						Printed by : <?php echo $uname[Name];?>
					</div>
					
				</div>
				<hr>
			<p>Analysis Date / Time : <?php echo convert_date($auth[t_date])."/".$auth[t_time];?> &nbsp;&nbsp;Authentication date / time :<?php echo convert_date($auth[d_date])."/".$auth[d_time];?>  </p>
			</div>
		</div>
	</div>
	-->
				...
				<div class="col-md-12">
					...
					<?php
						...
						if($note)
						{
							...
							echo "<p class='text-center' style='margin-top: 5px;position:absolute;right:40%'><b>----End of report----</b></p>";
						}
						else
						{
							echo "<p class='text-center' style='margin-top: 20px;position:absolute;right:40%'><b>----End of report----</b></p>";
						}
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-xs-12">
						<?php
							$doc_nm=1;
							$doc=mysqli_query($link, "select * from lab_doctor where category='1' order by sequence");
