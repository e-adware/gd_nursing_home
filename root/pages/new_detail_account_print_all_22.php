...
<body onafterprint="window.close();" onkeypress="close_window(event)" onunload="refreshParent()">
	<div class="container-fluid">
			...
			<div class="account_close_div">
				<?php
					...
					?>
						...
						<div class="noprint "><input type="button" class="btn btn-default" id="Name1" value="Print" onclick="close_window_child()">  <input type="button" class="btn btn-default" id="Name2" value="Exit" onclick="javascript:window.close()"></div>
					<?php
					}else
					{
						if($date==$date22)
				...
				?>
							<input type="button" value="<?php echo $close_btn; ?>" class="btn btn-danger" onClick="close_account('<?php echo $user_val; ?>','<?php echo $date22; ?>')" >
				<?php
					}
				?>
			</div>
		...
		<?php
			...
			$opd_visit_amount=$opd_reg_amount=$opd_disount_amount=0;
			$con_pay_qry=mysqli_query($link, " SELECT distinct `opd_id` FROM `consult_payment_detail` WHERE `date` between '$date1' and '$date2' $con_max_slno_str_less $con_max_slno_str_grtr $user ORDER BY `slno`");
			...
			if($lab_total_amount>0)
			{
				...
				while($inv_pay=mysqli_fetch_array($inv_pay_qry))
				{
					$pat_test_det_path_qry=mysqli_query($link," SELECT `test_rate` FROM `patient_test_details` WHERE `opd_id`='$inv_pay[opd_id]' AND `testid`  IN(SELECT `testid` FROM `testmaster` WHERE `category_id`='1') $user ");
					while($pat_test_det_path=mysqli_fetch_array($pat_test_det_path_qry))
					{
						$lab_path_amount+=$pat_test_det_path["test_rate"];
					}
					
					$pat_test_det_ecg_qry=mysqli_query($link," SELECT `test_rate` FROM `patient_test_details` WHERE `opd_id`='$inv_pay[opd_id]' AND `testid`  IN(SELECT `testid` FROM `testmaster` WHERE `category_id`='3' AND `type_id`='131') $user ");
					while($pat_test_det_ecg=mysqli_fetch_array($pat_test_det_ecg_qry))
					{
						$lab_ecg_amount+=$pat_test_det_ecg["test_rate"];
					}
					
					$pat_test_det_usg_qry=mysqli_query($link," SELECT `test_rate` FROM `patient_test_details` WHERE `opd_id`='$inv_pay[opd_id]' AND `testid`  IN(SELECT `testid` FROM `testmaster` WHERE `category_id`='2' AND `type_id`='128') $user ");
					while($pat_test_det_usg=mysqli_fetch_array($pat_test_det_usg_qry))
					{
						$lab_usg_amount+=$pat_test_det_usg["test_rate"];
					}
					
					$pat_test_det_xray_qry=mysqli_query($link," SELECT `test_rate` FROM `patient_test_details` WHERE `opd_id`='$inv_pay[opd_id]' AND `testid`  IN(SELECT `testid` FROM `testmaster` WHERE `category_id`='2' AND `type_id`='40') $user ");
