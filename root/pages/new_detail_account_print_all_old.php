...
// http://localhost/swasti_his/root/pages/new_detail_account_print_all_old.php?date1=2018-09-16&date2=2018-09-23&encounter=0&user_entry=198&pay_mode=0&EpMl=198&account_break=0
	...
	<div class="container-fluid">
		...
		<?php
			...
			if($encounter==0 || $encounter==1)
			{
				...
				while($con_pay=mysqli_fetch_array($con_pay_qry))
				{
					$pat_pay_det_opd=mysqli_fetch_array(mysqli_query($link," SELECT * FROM `consult_patient_payment_details` WHERE `opd_id`='$con_pay[opd_id]' "));
			...
			if($encounter==0 || $encounter==2)
			{
				...
				if($lab_total_amount>0)
				{
					...
					while($inv_pay=mysqli_fetch_array($inv_pay_qry))
					{
						...
						$pat_test_det_ecg_qry=mysqli_query($link," SELECT `test_rate` FROM `patient_test_details` WHERE `opd_id`='$inv_pay[opd_id]' AND `testid`  IN(SELECT `testid` FROM `testmaster` WHERE `type_id`='131') ");
						while($pat_test_det_ecg=mysqli_fetch_array($pat_test_det_ecg_qry))
						{
							$lab_ecg_amount+=$pat_test_det_ecg["test_rate"];
						}
						
						$pat_test_det_usg_qry=mysqli_query($link," SELECT `test_rate` FROM `patient_test_details` WHERE `opd_id`='$inv_pay[opd_id]' AND `testid`  IN(SELECT `testid` FROM `testmaster` WHERE `type_id`='128') $user ");
						while($pat_test_det_usg=mysqli_fetch_array($pat_test_det_usg_qry))
						{
							$lab_usg_amount+=$pat_test_det_usg["test_rate"];
						}
						
						$pat_test_det_xray_qry=mysqli_query($link," SELECT `test_rate` FROM `patient_test_details` WHERE `opd_id`='$inv_pay[opd_id]' AND `testid`  IN(SELECT `testid` FROM `testmaster` WHERE `type_id`='40')");
					...
					$all_total_amount+=$lab_total_amount;
			...
			// IPD , Caualty
			$only_ipd_amount=$only_casu_amount=0;
			...
			$all_total_amount+=$only_ipd_amount+$only_casu_amount;
