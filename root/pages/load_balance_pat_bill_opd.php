...
if($val=="000")
{
	$qry="select * from consult_patient_payment_details where balance>0 AND `opd_id` IN(SELECT `opd_id` FROM `uhid_and_opdid`) order by slno desc";
}
else if($val)
{
	$qry="select * from consult_patient_payment_details where opd_id like '$val%' AND `opd_id` IN(SELECT `opd_id` FROM `uhid_and_opdid`)";	
}
else
{
	$qry="select * from consult_patient_payment_details where `opd_id` IN(SELECT `opd_id` FROM `uhid_and_opdid`) order by slno desc limit 0,10";
}
...
<th>Slno</th><th>OPD ID</th><th>Name</th><th>Recv Date</th><th>Balance</th><th>User</th>
