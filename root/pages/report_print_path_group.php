...
<html lang="en">
		...
		<script>
			...
			function save_print_test(tst,opd_id,ipd_id,uhid,batch_no)
			{
				...
				function(data,status)
				{
					if($("#chk_page").val()>0)
					{
						window.opener.load_pat_info(''+data+'','update_p_sim');
					}
					else
					{
						var pin=opd_id;
						...
						window.opener.load_test_detail(uhid,pin,batch_no);
					}
					window.close();
				})
			}
	...
	<body onkeydown="close_window(event)" onafterprint="save_print_test('<?php echo $print_tst;?>','<?php echo $_GET['opd_id'];?>','<?php echo $_GET['ipd_id'];?>','<?php echo $_GET['uhid'];?>','<?php echo $_GET['batch_no'];?>');">
			...
			foreach($test1 as $tst)
			{
				...
				while($tt=mysqli_fetch_array($tsts))
				{
					...
					if(in_array($tt[testid],$test1))
					{
						...
						elseif($tt[testid]==806 || $tt[testid]==94) //----Semen annnnnalysis same as urine------//
							...
							<table class="table table-condensed">
								...
								<tr>
									<td valign="top" style="border-right:1px solid #CCC">
										<?php
											$sen=mysqli_query($link, "select a.* from testresults a,Parameter_old b where a.patient_id='$uhid' AND a.opd_id='$opd_id' and a.ipd_id='$ipd_id' and a.batch_no='$batch_no' and a.testid='$all_cult' and (a.result like 'S' or a.result like 's%') and a.paramid=b.ID and b.ResultOptionID='68' order by b.Name");
									...
									<td valign="top" style="border-right:1px solid #CCC">
										<?php
											$sen=mysqli_query($link, "select a.* from testresults a,Parameter_old b where a.patient_id='$uhid' AND a.opd_id='$opd_id' and a.ipd_id='$ipd_id' and a.batch_no='$batch_no' and a.testid='$all_cult' and (a.result like 'I%' or a.result like 'i%') and a.paramid=b.ID and b.ResultOptionID='68' order by b.Name");
											while($s=mysqli_fetch_array($sen))
											{
									...
									<td valign="top" style="border-right:1px solid #CCC">
										<?php
											$sen=mysqli_query($link, "select a.* from testresults a,Parameter_old b where a.patient_id='$uhid' AND a.opd_id='$opd_id' and a.ipd_id='$ipd_id' and a.batch_no='$batch_no'  and a.testid='$all_cult' and ( a.result like 'R%' or a.result like 'r%') and a.paramid=b.ID and b.ResultOptionID='68' order by b.Name");
											while($s=mysqli_fetch_array($sen))
											{
								...
								<div class="table-modifier">
									<?php
										...
										else
										{
											...
											{
												$summ_all=mysqli_fetch_array($chk_sum);
												$res_sum=$summ_all[summary];
											}
											
										}
										
										$sum_image=mysqli_query($link,"select * from image_temp where patient_id='$uhid' and `opd_id`='$opd_id' and `ipd_id`='$ipd_id' and `batch_no`='$batch_no' and testid='$ap' order by img_no");
										if(mysqli_num_rows($sum_image)>0)
										{
											?>
											<div class="row">
												<div class="span6"><?php echo $res_sum;?></div>
												<div class="span3">
												<?php
														while($sum_img=mysqli_fetch_array($sum_image))
														{
															?><img src="../<?php echo $sum_img[Path];?>" style="border:1px solid #CCC;width:500px;height:170px"/> <br/> <?php
														} ?>
												</div>
											</div>
											<?php
