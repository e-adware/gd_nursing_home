...
{
	$("#age").css({'border-color': '#555555'});
	var n=a.length;
	if(n>0)
	{
		$("#age").css({'border-color': '#55555'});
		$(".dob").val("");
		var unicode=e.keyCode? e.keyCode : e.charCode;
		if(unicode==89)
		{
...
{
	$("#police").css({'border-color': '#555555'});
	var unicode=e.keyCode? e.keyCode : e.charCode;
	if(unicode==13)
	{
		 if($("#police").val()=="")
...
function load_emp_details(uhid,typ)
{
	...
	if(typ=="uhid" || typ=="name" || typ=="phone" || typ=="opd_serial")
	{
		setTimeout(function(){
			window.location="processing.php?param=3&uhid="+uhid+"&consult=1"+param_str;
			//window.location="processing.php?param=3&uhid="+uhid+"&consult=1&opd="+opd_id;
		 }, 1000);
	}
	if(typ=="pin")
	{
