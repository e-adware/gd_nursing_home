			...
			}else
			{
				...
				setTimeout(function(){
					...
					if(q[0]=="Saved")
					{
						...
						window.location="processing.php?param=3&uhid="+$("#uhid").text().trim()+"&consult=1&opd="+q[1];
					}
					if(q[0]=="Updated")
					{
						$("#edit_div").html('');
			...
			function(data,status)
			{
				...
				if(str[0]=='Saved' || str[0]=='Update')
				{
					...
					setTimeout(function(){
						...
						window.location="processing.php?param=3&uhid="+str[2]+"&lab=1&opd="+str[1];
					},1000);
				}else
				{
					save_test(val,opd);
...
function addoc_load(id,name,fee,valid)
{
	$("#ad_doc").val(name+"-"+id);
	$("#con_doc_id").val(id);
	$("#con_doc_fee").val(fee);
