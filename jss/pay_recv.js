...
function load_pat_bill(val)
{
	if(val.length>0)
	{
		$.post("pages/load_balance_pat_bill.php",
		{
			val:val,
			chk:$("#chk").val(),
		},
...
{
    $.post("pages/patient_balance_info.php",
    {
        uhid:uhid,
        vis:vis
    },
    function(data,status)
    {
        ...
        $("html, body").animate({ scrollTop: 500 },"slow",function(){document.getElementById("discount_now").focus()})
...
function recv_payment()
{
    ...
    $.post("pages/patient_balance_recv.php",
    {
        discount:discount,
