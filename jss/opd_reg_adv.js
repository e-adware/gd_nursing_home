...
function adload_refdoc(val,e)
{
	...
	else
	{
		...
		addoc_load(docs[1],doc_naam,docs[3],docs[4]);
	}
}
function addoc_load(id,name,visit_fee,regd_fee)
{
	$("#ad_doc").val(name+"-"+id);
	$("#addoc_id").val(id);
	$("#addoc_info").html("");
	...
	$("#regd_fee_adv").text(regd_fee);
	$("#consult_fee_adv").text(visit_fee);
	var tot_fee=parseInt(regd_fee)+parseInt(visit_fee);
	$("#total_fee_adv").text(tot_fee);
}
